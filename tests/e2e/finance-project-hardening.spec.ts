...
test("visit defaults follow historical terms and manual overrides survive visit changes",async({page})=>{
  ...
  sql(`begin; select set_config('request.jwt.claim.sub','${actors[0].id}',true);
    select public.save_finance_project_terms(${studioLiteral},'${randomUUID()}','${projectId}','{"stream":"supervision","mode":"per_visit","revision":0,"amount":"3000","currency":"UAH","effectiveFrom":"2026-06-01","reason":"June rate"}');
    select public.save_finance_project_terms(${studioLiteral},'${randomUUID()}','${projectId}','{"stream":"supervision","mode":"per_visit","revision":1,"amount":"4000","currency":"USD","effectiveFrom":"2026-09-01","reason":"September rate"}');
  ...
  expect(sql(`select count(*) from public.finance_project_items where project_id='${projectId}' and source='visit'`)).toBe("3");
});
