...
test("value builder preserves protected payments and native-currency area history",async({page},testInfo)=>{
 ...
 await dialog.getByLabel(p.contract,{exact:true}).fill('1000.12');
 await dialog.getByRole('combobox',{name:p.currency,exact:true}).click();await page.getByRole('combobox',{name:t.searchCurrencies,exact:true}).fill('IQD');await page.getByRole('option',{name:'IQD',exact:true}).click();
 await expect(dialog.getByRole('combobox',{name:p.currency,exact:true})).toContainText('IQD');
 await dialog.getByRole('combobox',{name:p.currency,exact:true}).click();await page.getByRole('combobox',{name:t.searchCurrencies,exact:true}).fill('BOV');await page.getByRole('option',{name:'BOV',exact:true}).click();
 await expect(dialog.getByText(b.referenceUnavailable.replace('{currency}','BOV'),{exact:true}).last()).toBeVisible({timeout:10000});await expect(dialog.getByRole('button',{name:b.referenceRetry,exact:true}).last()).toBeVisible();
 await page.screenshot({path:testInfo.outputPath('missing-fx-fullhd-en-light.png')});
 await dialog.getByRole('combobox',{name:p.currency,exact:true}).click();await page.getByRole('option',{name:'EUR',exact:true}).click();
 await expect(dialog.getByLabel(p.contract,{exact:true})).toHaveValue('1000.12');
 await page.waitForTimeout(250);
 await page.screenshot({path:testInfo.outputPath('new-fixed-fullhd-en-light.png')});
 ...
 await expect(dialog.getByText('123 m² × 20 USD/m² = USD 2,460.00',{exact:true})).toBeVisible();
 await expect(dialog.getByText(/NBU rate/u).first()).toBeVisible({timeout:10000});
 await page.waitForTimeout(250);
 await page.screenshot({path:testInfo.outputPath('new-area-fullhd-en-light.png')});
 const scheduleMotion=dialog.locator('[data-template-transition] > div');
 await expect(scheduleMotion).toHaveCSS('transition-duration','0.22s');
 for(const [label,amounts] of [['100%',['2460.00']],['50 / 50',['1230.00','1230.00']],['30 / 50 / 20',['738.00','1230.00','492.00']],['25 / 25 / 25 / 25',['615.00','615.00','615.00','615.00']]] as const) {
  const templateButton=dialog.getByRole('button',{name:label,exact:true});
  if(label==='100%') {await templateButton.focus();await page.keyboard.press('Enter');await expect(templateButton).toBeFocused();}
  else await templateButton.click();
  await expect(dialog.locator('[data-plan-row]')).toHaveCount(amounts.length);
  await expect(dialog.getByRole('button',{name:label,exact:true})).toHaveAttribute('aria-pressed','true');
  for(const [i,amount] of amounts.entries()) await expect(dialog.locator('[data-plan-row]').nth(i).getByLabel(t.movements.amount,{exact:true})).toHaveAttribute('data-derived-amount',amount);
  await expect.poll(()=>scheduleMotion.evaluate(element=>element.getAnimations().some(animation=>animation.playState==='running')),{timeout:180,intervals:[16]}).toBe(true);
  await page.waitForTimeout(240);
 }
 expect(await scheduleMotion.evaluate(element=>Math.abs(element.clientHeight-element.scrollHeight)<=1)).toBe(true);
 await page.screenshot({path:testInfo.outputPath('template-four-rows-fullhd-en-light.png')});
 await dialog.getByRole('button',{name:'100%',exact:true}).click();await expect(dialog.locator('[data-plan-row]')).toHaveCount(1);await page.waitForTimeout(240);
 await dialog.getByRole('button',{name:b.custom,exact:true}).click();await expect(dialog.getByRole('button',{name:b.custom,exact:true})).toBeFocused();await expect(dialog.getByRole('button',{name:b.custom,exact:true})).toHaveAttribute('aria-pressed','true');await expect(dialog.locator('[data-plan-row]')).toHaveCount(1);
 await page.emulateMedia({reducedMotion:'reduce'});await expect(scheduleMotion).toHaveCSS('transition-property','none');await dialog.getByRole('button',{name:'50 / 50',exact:true}).click();await expect(dialog.locator('[data-plan-row]')).toHaveCount(2);await page.emulateMedia({reducedMotion:'no-preference'});
 ...
 await expect(dialog.getByText(b.fullyAllocated,{exact:true})).toBeVisible();
 ...
 for(const [label,width,height] of [['fullhd',1920,1080],['2k',2560,1440],['mobile',390,844]] as const) for(const language of ['en','uk'] as const) for(const theme of ['light','dark'] as const) {
  ...
  if(label==='mobile'&&language==='en'&&theme==='light') {
   ...
   const mobileMotion=dialog.locator('[data-template-transition] > div');
   const scrollPane=dialog.locator(':scope > .overflow-y-auto');
   await dialog.getByRole('button',{name:'100%',exact:true}).click();await expect(dialog.locator('[data-plan-row]')).toHaveCount(1);await page.waitForTimeout(240);
   await scrollPane.evaluate(element=>{element.scrollTop=340;});
   const scrollTop=await scrollPane.evaluate(element=>element.scrollTop);
   await dialog.getByRole('button',{name:'50 / 50',exact:true}).click();await expect(dialog.locator('[data-plan-row]')).toHaveCount(2);
   await expect.poll(()=>mobileMotion.evaluate(element=>element.getAnimations().some(animation=>animation.playState==='running')),{timeout:180,intervals:[16]}).toBe(true);
   await page.waitForTimeout(240);expect(await dialog.evaluate(el=>el.scrollWidth<=el.clientWidth)).toBe(true);expect(await scrollPane.evaluate(element=>element.scrollTop)).toBe(scrollTop);expect(await mobileMotion.evaluate(element=>Math.abs(element.clientHeight-element.scrollHeight)<=1)).toBe(true);
   await page.screenshot({path:testInfo.outputPath('template-two-rows-mobile-en-light.png')});
   await page.emulateMedia({colorScheme:theme,reducedMotion:'reduce'});
  }
  await page.screenshot({path:testInfo.outputPath(`builder-${label}-${language}-${theme}.png`)});
  await dialog.getByRole('button',{name:messages.builder.perAreaShort,exact:true}).click();
