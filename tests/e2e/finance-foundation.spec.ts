...
test("actual movements, transfers, historical FX, refunds and reversal", async ({ page }, testInfo) => {
  ...
  const transferRow=page.locator("li").filter({ has:page.getByText(t.kinds.transfer,{ exact:true }) }).first();expect(await transferRow.locator("summary .ui-numeric").getAttribute("class")).toContain("text-[var(--ui-text)]");
