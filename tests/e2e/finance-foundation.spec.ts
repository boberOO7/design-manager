...
for (const locale of ["en", "uk"] as const) {
  test(`${locale}: setup, precision, finalization, and archival`, async ({ page }, testInfo) => {
    ...
    let editAccount=page.getByRole("button", { name: t.editNamed.replace("{name}", "Operating bank") });await editAccount.focus();await expect(editAccount.locator("..")).toHaveCSS("opacity","1");await editAccount.click();
    ...
    await expect(dialog).toHaveCount(0);
    editAccount=page.getByRole("button", { name: t.editNamed.replace("{name}", "Operating bank") });await editAccount.focus();await editAccount.click();
    dialog = page.getByRole("dialog");
    ...
    await dialog.getByRole("button", { name: t.saveAccount }).click();
    await expect(dialog).toHaveCount(0);
    await page.getByRole("button", { name: t.archiveNamed.replace("{name}", "Renamed bank"), exact: true }).click();
    await page.getByText(t.archivedAccounts.replace("{count}", "1"), { exact: true }).click();
    const restoreAccount=page.getByRole("button", { name: t.restoreNamed.replace("{name}", "Renamed bank"), exact: true });
    await expect(restoreAccount).toBeVisible();
    await restoreAccount.click();
    ...
    await expect(dialog.getByLabel(t.openingBalance, { exact: true })).toHaveCount(0);
    await expect(dialog.locator('input[name="openingBalance"]')).toHaveValue("0");
    await dialog.getByRole("button", { name: t.saveAccount }).click();
    await expect(dialog).toHaveCount(0);
...
test("actual movements, transfers, historical FX, refunds and reversal", async ({ page }, testInfo) => {
  ...
  const financeNav=page.getByRole("navigation",{name:en.Finance.title});await expect(financeNav.getByRole("link",{name:en.Finance.accounts,exact:true})).toHaveCount(0);await financeNav.getByRole("button",{name:en.Finance.manageFinance,exact:true}).click();await expect(page.getByRole("menuitem",{name:en.Finance.accounts,exact:true})).toHaveAttribute("href","/finance/accounts");await expect(page.getByRole("menuitem",{name:en.Finance.planning.categories,exact:true})).toHaveAttribute("href","/finance/categories");await page.keyboard.press("Escape");
  ...
  const receiptRow=page.locator("li").filter({has:page.getByText("Design receipt",{exact:true})});const suppliesRow=page.locator("li").filter({has:page.getByText("Studio supplies",{exact:true})});await expect(receiptRow.locator("summary .ui-numeric")).toContainText("+");await expect(suppliesRow.locator("summary .ui-numeric")).toContainText("-");expect((await receiptRow.locator("summary").boundingBox())?.height).toBeLessThanOrEqual(64);expect(await receiptRow.locator("summary .ui-numeric").evaluate(element=>getComputedStyle(element).color)).not.toBe(await suppliesRow.locator("summary .ui-numeric").evaluate(element=>getComputedStyle(element).color));
  ...
  await expect(exchange.locator("summary .ui-numeric")).toContainText("→");
  await exchange.locator("summary").click();
  await expect(exchange.getByText(/USD\s*10\.00.*UAH\s*420\.00.*42/).first()).toBeVisible();
  await expect(exchange.getByText(/1 UAH = 1 UAH/)).toHaveCount(0);
  ...
  const receipt=page.locator("li").filter({ has:page.getByText("Design receipt",{ exact:true }) });
  await receipt.locator("summary").click();
  await expect(receipt.getByText(uk.Finance.planning.defaults.project_payments,{ exact:true })).toBeVisible();
...
test("categories, expected items, partial overdue settlement and contextual payment",async({ page },testInfo)=>{
  ...
  await expect(dialog.getByRole("combobox",{ name:p.expectedDate,exact:true })).toHaveCount(0);
  await expect(dialog.locator('input[name="expectedDate"]')).toHaveValue("2026-09-01");
  ...
  await expect(item.getByText(p.states.partial,{ exact:true })).toBeVisible();await expect(page.locator('section[aria-labelledby="expected-attention"]')).toContainText(p.attentionOverdue);
  ...
  await expect(dialog.getByRole("combobox",{ name:uk.Finance.planning.expectedDate,exact:true })).toHaveCount(0);
  await expect(dialog.getByRole("checkbox",{ name:uk.Finance.planning.differentExpectedDate,exact:true })).toBeVisible();
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);
