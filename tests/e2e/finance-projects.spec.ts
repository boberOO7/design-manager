...
test("project agreements, shared settlement, supervision, bonus and archived collections",async({page},testInfo)=>{
  ...
  async function createPayment(amount:string,name:string,dated=false){
    ...
    await dialog.getByLabel(t.movements.amount,{exact:true}).fill(amount);await dialog.getByText(plan.moreOptions,{exact:true}).click();await dialog.getByLabel(t.movements.description,{exact:true}).fill(name);
