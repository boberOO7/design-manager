...
test("project agreements, shared settlement, supervision, bonus and archived collections",async({page},testInfo)=>{
  ...
  async function createPayment(amount:string,name:string,dated=false){
    ...
    if(dated){await dialog.getByRole("combobox",{name:plan.dueDate,exact:true}).click();await dialog.getByRole("gridcell",{name:"1",exact:true}).first().click();await dialog.getByRole("checkbox",{name:plan.differentExpectedDate,exact:true}).check();await dialog.getByRole("combobox",{name:plan.expectedDate,exact:true}).click();await dialog.getByRole("gridcell",{name:"30",exact:true}).first().click();}
  ...
  await dialog.getByRole("combobox",{name:plan.payment,exact:true}).click();await page.getByRole("option",{name:/Bank receipt/}).click();await dialog.getByRole("button",{name:plan.match,exact:true}).click();await expect(dialog).toHaveCount(0);await expect(advance.getByText(plan.states.settled,{exact:true})).toBeVisible();
  expect(sql(`select count(*) from public.finance_movements where studio_id=${studioLiteral}`)).toBe("2");
  await page.getByRole("button",{name:p.editAgreement,exact:true}).click();dialog=page.getByRole("dialog");await dialog.getByLabel(p.contract,{exact:true}).fill("1200");await dialog.getByLabel(p.reason,{exact:true}).fill("Additional design scope");await dialog.getByRole("button",{name:t.builder.saveRevision,exact:true}).click();await expect(dialog).toHaveCount(0);
  expect(sql(`select concat(trim_scale(contract_amount),'|',trim_scale(scheduled_amount),'|',trim_scale(unscheduled_amount),'|',trim_scale(collected_amount)) from public.finance_project_totals where project_id='${projectId}' and stream='design'`)).toBe("1200|700|500|400");
