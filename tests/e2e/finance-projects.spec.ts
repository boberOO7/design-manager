...
test("project agreements, shared settlement, supervision, bonus and archived collections",async({page},testInfo)=>{
  ...
  await page.goto(projectHref);await advance.getByRole("button",{name:plan.match,exact:true}).click();dialog=page.getByRole("dialog");
