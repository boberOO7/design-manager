...
export function TextField({ defaultValue, error, initialFocus = false, label, name, required = false, type = "text" }: { defaultValue?: string | null; error?: string; initialFocus?: boolean; label: string; name: string; required?: boolean; type?: React.HTMLInputTypeAttribute }) {
  return <FormField label={label} error={error} optional={!required}><Input data-dialog-initial-focus={initialFocus || undefined} defaultValue={defaultValue ?? ""} name={name} required={required} type={type} aria-invalid={Boolean(error)} /></FormField>;
}
