...
function LeadFollowUpFields({ defaultStartDate, lead, state }: { defaultStartDate: string; lead: CrmLead | null; state: CrmActionState }) {
  ...
  return <FollowUpControls date={date} defaultStartDate={defaultStartDate} error={state.fieldErrors?.next_contact_date} onDateChange={setDate} onTimeChange={setTime} optional time={time} />;
}

function FollowUpControls({ date, defaultStartDate, error, initialFocus = false, onDateChange, onTimeChange, optional = false, time }: { date: string; defaultStartDate: string; error?: string; initialFocus?: boolean; onDateChange: (value: string) => void; onTimeChange: (value: string) => void; optional?: boolean; time: string }) {
  const t = useTranslations("Crm");
  const locale = useLocale();
  ...
  return <div className="grid gap-3">
    <div className="grid grid-cols-[minmax(0,1fr)_8rem] gap-2">
      <FormField as="div" label={t("followUp.date")} error={error} optional={optional}><DatePicker data-dialog-initial-focus={initialFocus || undefined} aria-label={t("followUp.date")} name="next_contact_date" min={defaultStartDate} value={date} onValueChange={onDateChange} locale={locale} invalid={Boolean(error)} /></FormField>
      <FormField as="div" label={t("followUp.time")}><TimePicker name="next_contact_time" value={time} onValueChange={onTimeChange} locale={locale} disabled={!date} aria-label={t("followUp.time")} /></FormField>
    </div>
