...
const eventTypeKey: Record<CalendarEventType, "meeting" | "presentation" | "siteVisit" | "internalReview" | "event"> = {
  meeting: "meeting",
...
function CalendarPill({ item, month = false, mobile = false, onClick }: { item: CalendarItem; month?: boolean; mobile?: boolean; onClick: () => void }) {
  ...
  const accessibleLabel = `${label}: ${!item.allDay && item.source === "calendar_event" ? `${formatCalendarTime(item.startsAt, locale)}, ` : ""}${item.title}`;
  return <button type="button" onClick={(event) => { event.stopPropagation(); onClick(); }} aria-label={accessibleLabel} className={month ? mobile ? `box-border block min-h-8 w-full appearance-none truncate rounded-md border-l-2 px-2 text-left text-xs font-medium leading-8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)] focus-visible:ring-offset-1 ${itemTone(item)}` : `block min-h-0 w-full appearance-none ${monthItemPresentationClassName} ${itemTone(item)}` : `min-h-10 w-full truncate rounded-md border-l-2 px-2 py-1 text-left text-xs font-medium ${itemTone(item)}`} style={monthStyle} title={accessibleLabel}>
    <span className="sr-only">{label}: </span>{!item.allDay && item.source === "calendar_event" ? `${formatCalendarTime(item.startsAt, locale)} ` : ""}{item.title}
  </button>;
}
  ...
  return <div className="min-w-0 space-y-5">
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      ...
      <div className="flex flex-wrap gap-2 sm:justify-end">
        {initialData.isAdmin ? <Button size="sm" className="min-h-11 sm:min-h-0" onClick={() => setDrawer({ kind: "event-form" })}><Plus className="size-4" />{t("addEvent")}</Button> : null}
        <Button size="sm" className="min-h-11 sm:min-h-0" variant="outline" onClick={() => setDrawer({ kind: "time-off-form" })}>{t("requestTimeOff")}</Button>
    ...
    {drawer?.kind === "day" ? <DetailPanel title={dateLabel(drawer.date, { weekday: "long", month: "long", day: "numeric", year: "numeric" }, locale)} eyebrow={t("dayDetails")} onClose={() => setDrawer(null)}>
      <DayDetails date={drawer.date} items={getDayItems(visibleItems, drawer.date)} onItem={(item) => setDrawer({ kind: "item", item })} />
      <div className="mt-6 flex flex-wrap gap-2">{initialData.isAdmin ? <Button size="sm" onClick={() => setDrawer({ kind: "event-form", date: drawer.date })}>{t("addEventOnDay")}</Button> : null}<Button size="sm" variant="outline" onClick={() => setDrawer({ kind: "time-off-form", date: drawer.date })}>{t("requestTimeOff")}</Button></div>
    </DetailPanel> : null}
...
function FilterBar({ data, filters, onChange }: { data: CalendarPageData; filters: CalendarFilters; onChange: (patch: Partial<CalendarFilters>) => void }) {
  ...
  return <div className="grid gap-3 border-b border-[var(--ui-border)] bg-[var(--ui-surface-subtle)] p-4 lg:grid-cols-[1.5fr_1fr_1fr_auto]">
    ...
    <select aria-label={t("filterProject")} className={fieldClass} value={filters.projectId} onChange={(event) => onChange({ projectId: event.target.value })}><option value="">{t("allProjects")}</option>{data.projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select>
    <select aria-label={t("filterPerson")} className={fieldClass} value={filters.personId} onChange={(event) => onChange({ personId: event.target.value })}><option value="">{t("allPeople")}</option>{data.people.map((person) => <option key={person.id} value={person.id}>{person.full_name}</option>)}</select>
...
function ItemPanel({ item, data, onClose, onEdit, onMutated }: { item: CalendarItem; data: CalendarPageData; onClose: () => void; onEdit: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void; onMutated: (item: CalendarItem | null, removedKey: string | null) => void }) {
  const t = useTranslations("Calendar"); const timeOff = useTranslations("TimeOff"); const roles = useTranslations("Roles"); const status = useTranslations("Status"); const priority = useTranslations("Priority");
  ...
  async function cancelEvent() { if (item.source !== "calendar_event" || !window.confirm(t("cancelEventConfirm"))) return; setPending(true); const response = await fetch(`/api/calendar/events/${encodeURIComponent(item.id)}`, { method: "DELETE" }); setPending(false); if (response.ok) onMutated(null, item.key); else setError(t("eventCancelFailed")); }
  ...
  return <DetailPanel title={item.title} eyebrow={itemEyebrow} onClose={pending ? () => undefined : onClose}>{error ? <p role="alert" className="mb-4 rounded-xl bg-[var(--ui-danger-surface)] p-3 text-sm text-[var(--ui-danger-text)]">{error}</p> : null}<div className="space-y-6 text-sm">
...
function EventForm({ data, item, initialDate, onClose, onSaved }: { data: CalendarPageData; item?: Extract<CalendarItem, { source: "calendar_event" }>; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void }) {
  ...
  return <DetailPanel title={item ? t("editEventTitle") : t("addEventTitle")} eyebrow={t("eventForm")} onClose={requestClose}><div className="space-y-4" aria-busy={pending}>
    ...
    <div className="grid gap-4 sm:grid-cols-2"><FormField label={t("type")} error={fieldErrors.eventType}><select className={fieldClass} value={values.eventType} onChange={(event) => setValues({ ...values, eventType: event.target.value as CalendarEventType, attendeeIds: [] })}>{CALENDAR_EVENT_TYPES.map((type) => <option key={type} value={type}>{t(eventTypeKey[type])}</option>)}</select></FormField><FormField label={t("project")} optional error={fieldErrors.projectId}><select className={fieldClass} value={values.projectId} onChange={(event) => setValues({ ...values, projectId: event.target.value, attendeeIds: [] })}><option value="">{t("studioWide")}</option>{data.projects.map((project) => <option key={project.id} value={project.id} disabled={project.status === "completed"}>{project.name}{project.status === "completed" ? ` (${t("completedReopen")})` : ""}</option>)}</select></FormField></div>
    ...
    <div className="grid gap-4 sm:grid-cols-2">{values.allDay ? <><FormField label={t("startDate")}><input type="date" className={fieldClass} value={values.startDate} onChange={(event) => setValues({ ...values, startDate: event.target.value })} /></FormField><FormField label={t("endDate")} error={fieldErrors.endsAt}><input type="date" className={fieldClass} min={values.startDate} value={values.endDate} onChange={(event) => setValues({ ...values, endDate: event.target.value })} /></FormField></> : <><FormField label={t("starts")}><div className="grid min-w-0 grid-cols-2 gap-2"><input aria-label={t("startDate")} type="date" className={`${fieldClass} min-w-0`} value={values.startDate} onChange={(event) => setValues({ ...values, startDate: event.target.value })} /><input aria-label={t("startTime")} type="time" className={`${fieldClass} min-w-0`} value={values.startTime} onChange={(event) => setValues({ ...values, startTime: event.target.value })} /></div></FormField><FormField label={t("ends")} error={fieldErrors.endsAt}><div className="grid min-w-0 grid-cols-2 gap-2"><input aria-label={t("endDate")} type="date" className={`${fieldClass} min-w-0`} min={values.startDate} value={values.endDate} onChange={(event) => setValues({ ...values, endDate: event.target.value })} /><input aria-label={t("endTime")} type="time" className={`${fieldClass} min-w-0`} value={values.endTime} onChange={(event) => setValues({ ...values, endTime: event.target.value })} /></div></FormField></>}</div>
    <FormField label={t("attendees")} error={fieldErrors.attendeeIds}><select multiple className={`${textareaClassName} min-h-32 h-auto p-2`} value={values.attendeeIds} onChange={(event) => setValues({ ...values, attendeeIds: Array.from(event.target.selectedOptions, (option) => option.value) })}>{appropriatePeople.map((person) => { const roleKey = getCanonicalRoleTranslationKey(person.job_title); return <option key={person.id} value={person.id}>{person.full_name} — {roleKey ? roles(roleKey) : person.job_title}</option>; })}</select><span className="text-xs font-normal text-[var(--ui-text-muted)]">{t("attendeesHelp")}</span></FormField>
...
function TimeOffForm({ data, initialDate, onClose, onSaved }: { data: CalendarPageData; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "time_off_request_admin" }>) => void }) {
  ...
  return <DetailPanel title={calendar("requestTimeOffTitle")} eyebrow={t("privateRequest")} onClose={requestClose}><div className="space-y-4" aria-busy={pending}>
    <Input label={t("requestType")} error={fieldErrors.requestType}><select className={fieldClass} value={values.requestType} onChange={(event) => setValues({ ...values, requestType: event.target.value as TimeOffRequestType })}>{TIME_OFF_REQUEST_TYPES.map((type) => <option key={type} value={type}>{t(timeOffRequestTypeKey[type])}</option>)}</select></Input>
    ...
    {!values.allDay ? <div className="grid gap-4 sm:grid-cols-2"><Input label={t("startTime")}><input type="time" className={fieldClass} value={values.startTime} onChange={(event) => setValues({ ...values, startTime: event.target.value })} /></Input><Input label={t("endTime")} error={fieldErrors.endDate}><input type="time" className={fieldClass} value={values.endTime} onChange={(event) => setValues({ ...values, endTime: event.target.value })} /></Input></div> : null}
