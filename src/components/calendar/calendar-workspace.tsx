...
function CalendarEventDetails({ item, data, pending, onRespond }: { item: Extract<CalendarItem, { source: "calendar_event" }>; data: CalendarPageData; pending: boolean; onRespond: (inviteId: string, status: "accepted" | "declined") => void }) {
  ...
  return <>
    ...
    {sections.has("assignee") && item.assignee ? <section><h3 className="font-semibold text-[var(--ui-text)]">{t(item.eventType === "interview" ? "interviewer" : "assignee")}</h3><div className="mt-2"><CalendarPersonDetail person={item.assignee} /></div></section> : null}
    ...
    {sections.has("organizer") ? <section><h3 className="font-semibold text-[var(--ui-text)]">{t("organizer")}</h3><div className="mt-2"><CalendarPersonDetail person={item.organizer} /></div></section> : null}
