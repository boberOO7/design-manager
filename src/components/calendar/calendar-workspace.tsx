...
import {
  ...
  getMonthItemTop, itemOccursOn, mergeCalendarItem, MONTH_EVENT_GEOMETRY, MONTH_LANE_GAP, MONTH_LANE_HEIGHT, parseDateOnly, WEEK_PIXELS_PER_MINUTE,
...
export function CalendarWorkspace({ initialData, initialView, initialDate, searchParams }: { initialData: CalendarPageData; initialView: CalendarView; initialDate: string; searchParams: SearchParams }) {
  ...
  return <div className="min-w-0 space-y-5">
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      <div><p className="text-sm font-medium text-[var(--ui-text-muted)]">Studio schedule · {APPLICATION_TIME_ZONE}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-[var(--ui-text)]">Calendar</h1><p className="mt-1 text-sm text-[var(--ui-text-muted)]">Events, live deadlines, and privacy-safe team availability.</p></div>
      <div className="flex flex-wrap gap-2 sm:justify-end">
        ...
        {initialData.isAdmin && initialData.pendingCount > 0 ? <span className="inline-flex items-center rounded-full bg-[var(--ui-violet-surface)] px-3 text-xs font-semibold text-[var(--ui-violet-text)]">{initialData.pendingCount} pending</span> : null}
      </div>
    </header>
...
function FilterBar({ data, filters, onChange }: { data: CalendarPageData; filters: CalendarFilters; onChange: (patch: Partial<CalendarFilters>) => void }) {
  ...
  return <div className="grid gap-3 border-b border-[var(--ui-border)] bg-[var(--ui-surface-subtle)] p-4 lg:grid-cols-[1.5fr_1fr_1fr_auto]">
    ...
    <label className="flex items-center gap-2 whitespace-nowrap text-sm text-[var(--ui-text-secondary)]"><input type="checkbox" checked={filters.mine} onChange={(event) => onChange({ mine: event.target.checked })} />Relevant to me</label>
  </div>;
}
...
function MonthView({ anchor, today, items, onDay, onItem }: { anchor: string; today: string; items: CalendarItem[]; onDay: (date: string) => void; onItem: (item: CalendarItem) => void }) {
  ...
  return <><div className="hidden grid-cols-7 border-b border-[var(--ui-border)] text-center text-xs font-semibold uppercase tracking-wide text-[var(--ui-text-muted)] md:grid">{["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => <div key={day} className="py-3">{day}</div>)}</div>
...
function WeekView({ anchor, items, onItem }: { anchor: string; items: CalendarItem[]; onItem: (item: CalendarItem) => void }) {
  ...
  return <div className="relative"><p id="week-scroll-hint" className="border-b border-[var(--ui-border-subtle)] px-3 py-2 text-xs text-[var(--ui-text-muted)] md:hidden">Swipe sideways to see all days. Times stay on the left.</p><div ref={horizontalScrollRef} aria-describedby="week-scroll-hint" aria-label="Weekly calendar. Scroll horizontally to see all seven days." className="overflow-x-auto overscroll-x-contain"><div className="min-w-[840px] sm:min-w-[900px]">
    ...
    <div className="grid grid-cols-[3.5rem_repeat(7,minmax(7rem,1fr))] border-b border-[var(--ui-border)]">
      <div className="border-r border-[var(--ui-border-subtle)] px-2 py-2 text-[10px] font-semibold uppercase tracking-wide text-[var(--ui-text-subtle)]">All-day</div>
    ...
    <div ref={scrollRef} className="max-h-[36rem] overflow-y-auto">
      <div className="grid grid-cols-[3.5rem_repeat(7,minmax(7rem,1fr))]">
        ...
        {dates.map((date, dayIndex) => <div key={date} className={`relative border-r border-[var(--ui-border-subtle)] ${dayIndex === currentTime?.dayIndex ? "bg-[var(--ui-surface-subtle)]" : ""}`} style={{ height: 24 * 60 * WEEK_PIXELS_PER_MINUTE, backgroundImage: "repeating-linear-gradient(to bottom, transparent 0, transparent 59px, var(--ui-calendar-gridline) 60px)", backgroundSize: `100% ${60 * WEEK_PIXELS_PER_MINUTE}px` }}>
          {(timedByDate.get(date) ?? []).map((segment) => { const timeLabel = segment.item.source === "calendar_event" ? `${formatCalendarTime(segment.item.startsAt)}–${formatCalendarTime(segment.item.endsAt)}` : `${segment.item.startTime}–${segment.item.endTime}`; return <button key={segment.segmentId} type="button" onClick={() => onItem(segment.item)} title={segment.item.title} aria-label={`${segment.item.title}, ${timeLabel}`} className={`absolute overflow-hidden border-l-2 px-2 py-1 text-left text-xs font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--ui-focus)] ${itemTone(segment.item)}`} style={{ top: segment.startMinute * WEEK_PIXELS_PER_MINUTE, height: getTimedEventHeight(segment.startMinute, segment.endMinute), left: `calc(${(segment.column / segment.columnCount) * 100}% + 2px)`, width: `calc(${100 / segment.columnCount}% - 4px)` }}><span className="block truncate">{segment.item.title}</span><span className="block truncate text-[10px] font-normal opacity-80">{timeLabel}{segment.item.source === "calendar_event" && segment.item.location ? ` · ${segment.item.location}` : ""}</span></button>; })}
          {currentTime?.dayIndex === dayIndex ? <div className="pointer-events-none absolute z-10 inset-x-0 border-t-2 border-[var(--ui-danger-solid)]" style={{ top: currentTime.minute * WEEK_PIXELS_PER_MINUTE }} aria-label={`Current time: ${String(Math.floor(currentTime.minute / 60)).padStart(2, "0")}:${String(currentTime.minute % 60).padStart(2, "0")} Kyiv`}><span className="absolute -left-1 -top-1.5 size-3 rounded-full bg-[var(--ui-danger-surface)]0" /></div> : null}
        </div>)}
      </div>
    </div>
...
function AgendaView({ start, items, onItem }: { start: string; items: CalendarItem[]; onItem: (item: CalendarItem) => void }) {
  ...
  if (!dates.length) return <p className="p-10 text-center text-sm text-[var(--ui-text-muted)]">No visible Calendar items in this 30-day range.</p>;
  return <div className="grid gap-x-8 p-3 sm:p-4 lg:grid-cols-2">{dates.map((date) => <section key={date} className="border-b border-[var(--ui-border-subtle)] py-3 sm:py-4"><h3 className="mb-2 text-sm font-semibold text-[var(--ui-text)]">{dateLabel(date, { weekday: "long", month: "long", day: "numeric" })}</h3><div className="space-y-2">{getDayItems(items, date).map((item) => <CalendarPill key={item.key} item={item} onClick={() => onItem(item)} />)}</div></section>)}</div>;
}
...
function DayDetails({ items, onItem }: { date: string; items: CalendarItem[]; onItem: (item: CalendarItem) => void }) {
  if (!items.length) return <p className="rounded-xl border border-dashed border-[var(--ui-border-strong)] p-6 text-center text-sm text-[var(--ui-text-muted)]">Nothing scheduled.</p>;
...
function ItemPanel({ item, data, onClose, onEdit, onMutated }: { item: CalendarItem; data: CalendarPageData; onClose: () => void; onEdit: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void; onMutated: (item: CalendarItem | null, removedKey: string | null) => void }) {
  ...
  return <DetailPanel title={item.title} eyebrow={itemLabel(item)} onClose={pending ? () => undefined : onClose}>{error ? <p role="alert" className="mb-4 rounded-xl bg-[var(--ui-danger-surface)] p-3 text-sm text-[var(--ui-danger-text)]">{error}</p> : null}<div className="space-y-6 text-sm">
    <section><h3 className="font-semibold text-[var(--ui-text)]">When</h3><p className="mt-2 text-[var(--ui-text-secondary)]">{item.source === "calendar_event" ? `${formatCalendarDateTime(item.startsAt)} – ${formatCalendarDateTime(item.endsAt)}` : `${dateLabel(item.startDate, { month: "long", day: "numeric", year: "numeric" })}${item.endDate !== item.startDate ? ` – ${dateLabel(item.endDate, { month: "long", day: "numeric", year: "numeric" })}` : ""}`}</p></section>
    {item.source === "calendar_event" ? <><section><h3 className="font-semibold">Details</h3><p className="mt-2 whitespace-pre-wrap leading-6 text-[var(--ui-text-secondary)]">{item.description || "No description"}</p>{item.location ? <p className="mt-3 flex gap-2"><MapPin className="size-4" />{item.location}</p> : null}{item.meetingUrl ? <a className="mt-2 flex gap-2 underline" href={item.meetingUrl} target="_blank" rel="noreferrer"><Video className="size-4" />Open meeting link</a> : null}</section><section><h3 className="font-semibold">Attendees</h3><p className="mt-2 text-[var(--ui-text-secondary)]">{item.attendees.map((person) => person.full_name).join(", ") || "No attendees"}</p></section>{item.project ? <Link className="font-medium underline" href={`/projects/${item.project.id}`}>Open {item.project.name}</Link> : null}{data.isAdmin ? <div className="flex gap-2"><Button disabled={pending} onClick={() => onEdit(item)}>Edit event</Button><Button disabled={pending} variant="outline" onClick={() => void cancelEvent()}>Cancel event</Button></div> : null}</> : null}
    {item.source === "project_deadline" ? <><p className="text-[var(--ui-text-secondary)]">{item.project.clientName ?? "Project milestone"} · {item.project.status}</p><Link className="font-medium underline" href={`/projects/${item.project.id}`}>Open project</Link></> : null}
    {item.source === "task_deadline" ? <><p className="whitespace-pre-wrap text-[var(--ui-text-secondary)]">{item.task.description || "No task description"}</p><dl className="grid grid-cols-2 gap-4"><div><dt className="text-[var(--ui-text-muted)]">Assignee</dt><dd>{item.task.assigneeName}</dd></div><div><dt className="text-[var(--ui-text-muted)]">Status</dt><dd>{getTaskStatusLabel(item.task.status)}</dd></div><div><dt className="text-[var(--ui-text-muted)]">Priority</dt><dd>{getTaskPriorityLabel(item.task.priority)}</dd></div></dl><Link className="font-medium underline" href={`/projects/${item.task.projectId}`}>Open task in {item.task.projectName}</Link></> : null}
    {item.source === "time_off" ? <p className="text-[var(--ui-text-secondary)]">{item.subjectName} is out of office. Private request details are not shared.</p> : null}
    {item.source === "time_off_request_admin" ? <><dl className="grid grid-cols-2 gap-4"><div><dt className="text-[var(--ui-text-muted)]">Employee</dt><dd>{item.subjectName}</dd></div><div><dt className="text-[var(--ui-text-muted)]">Status</dt><dd><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${getTimeOffStatusBadgeStyle(item.status).className}`}>{getTimeOffStatusBadgeStyle(item.status).label}</span></dd></div><div><dt className="text-[var(--ui-text-muted)]">Request type</dt><dd>{requestLabels[item.requestType]}</dd></div><div><dt className="text-[var(--ui-text-muted)]">Time</dt><dd>{item.allDay ? "All day" : `${item.startTime}–${item.endTime}`}</dd></div></dl><section><h3 className="font-semibold">Private note</h3><p className="mt-2 whitespace-pre-wrap text-[var(--ui-text-secondary)]">{item.privateNote || "No private note"}</p></section>{item.reviewNote ? <section><h3 className="font-semibold">Review note</h3><p className="mt-2 text-[var(--ui-text-secondary)]">{item.reviewNote}</p></section> : null}{data.isAdmin && item.status === "pending" ? <section className="space-y-3 border-t border-[var(--ui-border-subtle)] pt-5"><label className="grid gap-1.5 font-medium">Review note<textarea value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} maxLength={2000} rows={3} className="rounded-xl border border-[var(--ui-border)] p-3 font-normal" /></label><div className="flex gap-2"><Button disabled={pending} onClick={() => void timeOffAction("approve")}>Approve</Button><Button disabled={pending} variant="outline" onClick={() => void timeOffAction("reject")}>Reject</Button></div></section> : null}{((item.isOwn && item.status === "pending") || (data.isAdmin && item.status !== "cancelled")) ? <Button disabled={pending} variant="outline" onClick={() => void timeOffAction("cancel")}>Cancel request</Button> : null}</> : null}
  </div></DetailPanel>;
}
