...
import { createCalendarEventFormValues, getWorkMakeupTitle, toCalendarEventMutationPayload, updateEventStartDate, updateEventStartTime } from "@/lib/calendar-event-form";
...
function EventForm({ isOpen, onExited, data, item, initialDate, onClose, onSaved }: { isOpen: boolean; onExited: () => void; data: CalendarPageData; item?: Extract<CalendarItem, { source: "calendar_event" }>; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void }) {
  ...
  function updateContext(patch: Partial<typeof values>) {
    const next = {
      ...values,
      ...patch,
      ...(patch.eventType === "work_makeup" ? { attendeeIds: [], location: "", meetingUrl: "", projectId: "", recurrenceRule: null } : {}),
  ...
  async function submit() {
    ...
    try {
      ...
      const payload = {
        ...
        title: isWorkMakeup ? getWorkMakeupTitle(values, locale, selectedDayOff ? { ...selectedDayOff, previousContributionMinutes } : null) : values.title,
