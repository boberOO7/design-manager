...
function CalendarPill({ item, month = false, onClick }: { item: CalendarItem; month?: boolean; onClick: () => void }) {
  ...
  return <button type="button" onClick={(event) => { event.stopPropagation(); onClick(); }} className={month ? `box-border block h-5 min-h-0 w-full appearance-none overflow-hidden border-y-0 border-r-0 p-0 text-left text-xs font-medium leading-5 focus:outline-none focus:ring-2 focus:ring-stone-900 focus:ring-offset-1 ${itemTone(item)}` : `w-full truncate rounded-md border-l-2 px-2 py-1 text-left text-xs font-medium ${itemTone(item)}`} style={monthStyle} title={`${itemLabel(item)}: ${item.title}`}>
    ...
    <section className="rounded-2xl border border-stone-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-stone-200 p-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <SegmentedControl ariaLabel="Calendar view" items={[{ value: "month", label: "Month" }, { value: "week", label: "Week" }, { value: "agenda", label: "Agenda" }]} value={initialView} onValueChange={(view) => navigate({ view })} />
...
function DetailPanel({ title, eyebrow, onClose, children }: { title: string; eyebrow: string; onClose: () => void; children: React.ReactNode }) {
  return <Drawer isOpen onClose={onClose} title={title} className="w-full max-w-[34rem]"><header className="flex items-start justify-between gap-4 border-b border-stone-100 px-5 py-4"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-stone-500">{eyebrow}</p><h2 className="mt-1 text-xl font-semibold text-stone-950">{title}</h2></div><Button size="sm" variant="ghost" className="size-11 p-0" onClick={onClose} aria-label="Close"><X className="size-4" /></Button></header><main className="min-h-0 flex-1 overflow-y-auto p-5">{children}</main></Drawer>;
}
