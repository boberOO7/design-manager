...
import {
  ...
  getVisibleDayItems, instantToWallInput, itemOccursOn, mergeCalendarItem, parseDateOnly,
  removeCalendarItem, startOfMondayWeek, toDateOnly, zonedWallTimeToIso,
} from "@/lib/calendar";
...
function EventForm({ data, item, initialDate, onClose, onSaved }: { data: CalendarPageData; item?: Extract<CalendarItem, { source: "calendar_event" }>; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void }) {
  const baseDate = initialDate ?? data.today; const initial = { title: item?.title ?? "", eventType: item?.eventType ?? "meeting" as CalendarEventType, projectId: item?.projectId ?? "", allDay: item?.allDay ?? false, startsAt: item ? instantToWallInput(item.startsAt) : `${baseDate}T09:00`, endsAt: item ? instantToWallInput(item.endsAt) : `${baseDate}T10:00`, attendeeIds: item?.attendees.map((person) => person.id) ?? [], location: item?.location ?? "", meetingUrl: item?.meetingUrl ?? "", description: item?.description ?? "" };
  ...
  async function submit() { setPending(true); setError(""); setFieldErrors({}); try { const response = await fetch(item ? `/api/calendar/events/${encodeURIComponent(item.id)}` : "/api/calendar/events", { method: item ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...values, projectId: values.projectId || null, startsAt: zonedWallTimeToIso(values.startsAt), endsAt: zonedWallTimeToIso(values.endsAt) }) }); const result: unknown = await response.json(); if (response.ok && isMutationResult(result) && result.item?.source === "calendar_event") onSaved(result.item); else if (typeof result === "object" && result !== null) { if ("formError" in result && typeof result.formError === "string") setError(result.formError); if ("fieldErrors" in result && typeof result.fieldErrors === "object" && result.fieldErrors !== null) setFieldErrors(Object.fromEntries(Object.entries(result.fieldErrors).filter((entry): entry is [string, string] => typeof entry[1] === "string"))); } } catch { setError("The event could not be saved."); } finally { setPending(false); } }
  return <DetailPanel title={item ? "Edit event" : "Add event"} eyebrow="Calendar event" onClose={requestClose}><div className="space-y-4"><Input label="Title" error={fieldErrors.title}><input className={fieldClass} value={values.title} onChange={(event) => setValues({ ...values, title: event.target.value })} /></Input><div className="grid gap-4 sm:grid-cols-2"><Input label="Type"><select className={fieldClass} value={values.eventType} onChange={(event) => setValues({ ...values, eventType: event.target.value as CalendarEventType, attendeeIds: [] })}>{CALENDAR_EVENT_TYPES.map((type) => <option key={type} value={type}>{eventLabels[type]}</option>)}</select></Input><Input label="Project (optional)"><select className={fieldClass} value={values.projectId} onChange={(event) => setValues({ ...values, projectId: event.target.value, attendeeIds: [] })}><option value="">Studio-wide</option>{data.projects.map((project) => <option key={project.id} value={project.id} disabled={project.status === "completed"}>{project.name}{project.status === "completed" ? " (completed — reopen first)" : ""}</option>)}</select></Input></div><label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={values.allDay} onChange={(event) => setValues({ ...values, allDay: event.target.checked })} />All-day event</label><div className="grid gap-4 sm:grid-cols-2"><Input label="Starts"><input type="datetime-local" className={fieldClass} value={values.startsAt} onChange={(event) => setValues({ ...values, startsAt: event.target.value })} /></Input><Input label="Ends" error={fieldErrors.endsAt}><input type="datetime-local" className={fieldClass} value={values.endsAt} onChange={(event) => setValues({ ...values, endsAt: event.target.value })} /></Input></div><Input label="Attendees"><select multiple className="min-h-32 w-full rounded-xl border border-stone-200 p-2 text-sm" value={values.attendeeIds} onChange={(event) => setValues({ ...values, attendeeIds: Array.from(event.target.selectedOptions, (option) => option.value) })}>{appropriatePeople.map((person) => <option key={person.id} value={person.id}>{person.full_name} — {person.job_title}</option>)}</select><span className="text-xs font-normal text-stone-500">Use Ctrl/Cmd to select multiple people. Non-meeting project events are limited to active project members.</span></Input><div className="grid gap-4 sm:grid-cols-2"><Input label="Location"><input className={fieldClass} value={values.location} onChange={(event) => setValues({ ...values, location: event.target.value })} /></Input><Input label="Meeting URL" error={fieldErrors.meetingUrl}><input type="url" className={fieldClass} value={values.meetingUrl} onChange={(event) => setValues({ ...values, meetingUrl: event.target.value })} /></Input></div><Input label="Description"><textarea className="w-full rounded-xl border border-stone-200 p-3 text-sm" rows={5} value={values.description} onChange={(event) => setValues({ ...values, description: event.target.value })} /></Input>{error ? <p role="alert" className="rounded-xl bg-red-50 p-3 text-sm text-red-800">{error}</p> : null}<div className="flex justify-end gap-2 border-t border-stone-100 pt-4"><Button variant="outline" disabled={pending} onClick={requestClose}>Cancel</Button><Button disabled={pending} onClick={() => void submit()}>{pending ? "Saving…" : "Save event"}</Button></div></div></DetailPanel>;
}
...
function TimeOffForm({ data, initialDate, onClose, onSaved }: { data: CalendarPageData; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "time_off_request_admin" }>) => void }) {
  const date = initialDate ?? data.today; const initial = { userId: data.currentUserId, requestType: "vacation" as TimeOffRequestType, startDate: date, endDate: date, allDay: true, startTime: "09:00", endTime: "10:00", privateNote: "" }; const [values, setValues] = useState(initial); const [pending, setPending] = useState(false); const [error, setError] = useState(""); const [fieldErrors, setFieldErrors] = useState<Record<string, string>>({}); const dirty = JSON.stringify(values) !== JSON.stringify(initial);
