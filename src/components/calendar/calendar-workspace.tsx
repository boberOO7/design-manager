...
import { createCalendarEventFormValues, getSiteVisitTitle, getWorkMakeupTitle, toCalendarEventMutationPayload, updateEventStartDate, updateEventStartTime } from "@/lib/calendar-event-form";
...
function EventForm({ isOpen, onExited, data, item, initialDate, onClose, onSaved }: { isOpen: boolean; onExited: () => void; data: CalendarPageData; item?: Extract<CalendarItem, { source: "calendar_event" }>; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void }) {
  ...
  function updateContext(patch: Partial<typeof values>) {
    const next = {
      ...values,
      ...patch,
      ...(patch.eventType === "work_makeup" ? { attendeeIds: [], location: "", meetingUrl: "", projectId: "", recurrenceRule: null, assigneeId: "" } : {}),
      ...(patch.eventType === "site_visit" ? { attendeeIds: [], meetingUrl: "", allDay: false, recurrenceRule: null, assigneeId: data.isAdmin ? "" : data.currentUserId } : {}),
  ...
  async function submit() {
    ...
    try {
      ...
      const payload = {
        ...
        title: isWorkMakeup ? getWorkMakeupTitle(values, locale, selectedDayOff ? { ...selectedDayOff, previousContributionMinutes } : null) : isSiteVisit ? getSiteVisitTitle(data.projects.find((project) => project.id === values.projectId)?.name ?? "", locale) : values.title,
  ...
  return <DetailPanel isOpen={isOpen} onExited={onExited} title={item ? t("editEventTitle") : t("addEventTitle")} eyebrow={t("eventForm")} onClose={requestClose}><form autoComplete="off" className="space-y-4" aria-busy={pending} onSubmit={(event) => event.preventDefault()}>
    {!isWorkMakeup && !isSiteVisit ? <FormField label={t("titleLabel")} error={fieldErrors.title}><input className={fieldClass} value={values.title} onChange={(event) => setValues({ ...values, title: event.target.value })} /></FormField> : null}
    <div className={isWorkMakeup || isSiteVisit ? "grid gap-4" : "grid gap-4 sm:grid-cols-2"}><FormField label={t("type")} error={fieldErrors.eventType}><Select value={values.eventType} onValueChange={(eventType) => updateContext({ eventType: eventType as CalendarEventType })}>{[...new Set([...allowedEventTypes, item?.eventType].filter((type): type is CalendarEventType => Boolean(type)))].map((type) => <SelectItem key={type} value={type}>{type === "meeting" ? t(data.isAdmin ? "meetingPresentation" : "officeMeetingPresentation") : t(eventTypeKey[type])}</SelectItem>)}</Select></FormField>{!isWorkMakeup ? <FormField label={t("project")} optional={!isSiteVisit} error={fieldErrors.projectId}><Select placeholder={t("selectProject")} value={values.projectId || undefined} onValueChange={(projectId) => updateContext({ projectId, ...(isSiteVisit ? { assigneeId: data.isAdmin ? "" : data.currentUserId } : {}) })}>{(isSiteVisit ? siteVisitProjects : data.projects).map((project) => <SelectItem key={project.id} value={project.id} disabled={project.status === "completed"}>{project.name}{project.status === "completed" ? ` (${t("completedReopen")})` : ""}</SelectItem>)}</Select>{values.projectId && !isSiteVisit ? <button type="button" onClick={() => updateContext({ projectId: "" })} className="mt-2 text-left text-xs font-medium text-[var(--ui-text-secondary)] underline">{t("clearProject")}</button> : null}</FormField> : null}</div>
    {isSiteVisit && data.isAdmin ? <FormField label={t("assignee")} error={fieldErrors.assigneeId}><Select disabled={!values.projectId} placeholder={values.projectId ? t("selectAssignee") : t("selectProjectFirst")} value={values.assigneeId || undefined} onValueChange={(assigneeId) => setValues({ ...values, assigneeId })}>{siteVisitAssignees.map((person) => <SelectItem key={person.id} value={person.id}>{person.full_name}</SelectItem>)}</Select></FormField> : null}
    ...
    {isWorkMakeup ? <label className="flex min-h-11 items-center gap-2 text-sm font-medium"><input type="checkbox" checked={values.allDay} onChange={(event) => setValues({ ...values, allDay: event.target.checked })} />{t("allDayEvent")}</label> : !isSiteVisit ? <RecurrenceControl allDayControl={<label className="flex min-h-11 items-center gap-2 text-sm font-medium"><input type="checkbox" checked={values.allDay} onChange={(event) => setValues({ ...values, allDay: event.target.checked })} />{t("allDayEvent")}</label>} locale={locale} value={values.recurrenceRule ?? null} onChange={(recurrenceRule) => setValues({ ...values, recurrenceRule })} /> : null}
    ...
    {!isWorkMakeup && !isSiteVisit && item?.occurrenceStart ? <FormField label={locale.startsWith("uk") ? "Застосувати зміни до" : "Apply changes to"}><Select value={scope} onValueChange={(nextScope) => setScope(nextScope as "this" | "series")}><SelectItem value="this">{locale.startsWith("uk") ? "Лише ця подія" : "Only this event"}</SelectItem><SelectItem value="series">{locale.startsWith("uk") ? "Уся серія" : "Entire series"}</SelectItem></Select></FormField> : null}
    {!isWorkMakeup && !isSiteVisit ? <><FormField as="div" label={t("invitees")} error={fieldErrors.attendeeIds}><InviteePicker people={appropriatePeople} selectedIds={values.attendeeIds} onChange={(attendeeIds) => setValues({ ...values, attendeeIds })} /><span className="text-xs font-normal text-[var(--ui-text-muted)]">{t("inviteesHelp")}</span></FormField><div className="grid gap-4 sm:grid-cols-2"><FormField label={t("location")}><input className={fieldClass} value={values.location} onChange={(event) => setValues({ ...values, location: event.target.value })} /></FormField><FormField label={t("meetingUrl")} error={fieldErrors.meetingUrl}><input type="url" className={fieldClass} value={values.meetingUrl} onChange={(event) => setValues({ ...values, meetingUrl: event.target.value })} /></FormField></div></> : isSiteVisit ? <FormField label={t("location")} optional><input className={fieldClass} value={values.location} onChange={(event) => setValues({ ...values, location: event.target.value })} /></FormField> : null}
