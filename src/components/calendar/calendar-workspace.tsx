...
import { Check, ChevronLeft, ChevronRight, Filter, MapPin, Plus, Search, Video, X } from "lucide-react";
...
function ItemPanel({ item, data, onClose, onEdit, onMutated }: { item: CalendarItem; data: CalendarPageData; onClose: () => void; onEdit: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void; onMutated: (item: CalendarItem | null, removedKey: string | null) => void }) {
  const t = useTranslations("Calendar"); const timeOff = useTranslations("TimeOff"); const notifications = useTranslations("Notifications"); const status = useTranslations("Status"); const priority = useTranslations("Priority");
  const itemTitle = useCalendarItemTitle();
...
function InviteePicker({ people, selectedIds, onChange }: { people: CalendarPerson[]; selectedIds: string[]; onChange: (ids: string[]) => void }) {
  const t = useTranslations("Calendar");
  ...
  return <div className="space-y-2"><div className="flex flex-wrap gap-2">{selected.map((person) => <span key={person.id} className="inline-flex min-h-8 items-center gap-1 rounded-full border border-[var(--ui-border)] bg-[var(--ui-surface-muted)] py-1 pl-1 pr-2 text-sm"><UserAvatar decorative imageUrl={person.avatar_url} name={person.full_name} /><span className="max-w-40 truncate">{person.full_name}</span><button type="button" className="ml-0.5 rounded-full p-0.5 hover:bg-[var(--ui-surface)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]" aria-label={t("removeInvitee", { name: person.full_name })} onClick={() => toggle(person.id)}><X className="size-3" /></button></span>)}</div><Popover.Root open={open} onOpenChange={setOpen}><Popover.Trigger asChild><button type="button" className="inline-flex min-h-11 items-center gap-2 rounded-[var(--ui-radius-control)] border border-dashed border-[var(--ui-border-strong)] px-3 text-sm font-medium text-[var(--ui-text-secondary)] hover:bg-[var(--ui-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]"><Plus className="size-4" />{t("addInvitees")}</button></Popover.Trigger><Popover.Portal><Popover.Content align="start" sideOffset={6} className="z-[80] w-[min(24rem,calc(100vw-2rem))] rounded-[var(--ui-radius-control)] border border-[var(--ui-border-strong)] bg-[var(--ui-surface)] p-2 shadow-[var(--ui-shadow-popover)]"><label className="flex h-11 items-center gap-2 rounded-lg border border-[var(--ui-border)] px-3"><Search className="size-4 text-[var(--ui-text-muted)]" /><input autoFocus className="min-w-0 flex-1 bg-transparent text-sm outline-none" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("searchInvitees")} /></label><div className="mt-2 max-h-64 overflow-y-auto">{visible.length ? visible.map((person) => <button type="button" key={person.id} onClick={() => toggle(person.id)} className="flex min-h-11 w-full items-center gap-3 rounded-lg px-2 text-left hover:bg-[var(--ui-surface-muted)]"><UserAvatar decorative imageUrl={person.avatar_url} name={person.full_name} /><span className="min-w-0 flex-1 truncate text-sm">{person.full_name}</span>{selectedIds.includes(person.id) ? <Check className="size-4 text-[var(--ui-action-primary)]" /> : null}</button>) : <p className="p-3 text-sm text-[var(--ui-text-muted)]">{t("noInviteeResults")}</p>}</div></Popover.Content></Popover.Portal></Popover.Root></div>;
}
...
function EventForm({ data, item, initialDate, onClose, onSaved }: { data: CalendarPageData; item?: Extract<CalendarItem, { source: "calendar_event" }>; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void }) {
  ...
  return <DetailPanel title={item ? t("editEventTitle") : t("addEventTitle")} eyebrow={t("eventForm")} onClose={requestClose}><form autoComplete="off" className="space-y-4" aria-busy={pending} onSubmit={(event) => event.preventDefault()}>
    ...
    <div className="grid gap-4 sm:grid-cols-2">{values.allDay ? <><FormField label={t("startDate")}><DatePicker locale={locale} value={values.startDate} onValueChange={(startDate) => setValues({ ...values, startDate })} /></FormField><FormField label={t("endDate")} error={fieldErrors.endsAt}><DatePicker locale={locale} min={values.startDate} value={values.endDate} invalid={Boolean(fieldErrors.endsAt)} onValueChange={(endDate) => setValues({ ...values, endDate })} /></FormField></> : <><FormField label={t("starts")}><div className="grid min-w-0 grid-cols-2 gap-2"><DatePicker aria-label={t("startDate")} locale={locale} value={values.startDate} onValueChange={(startDate) => setValues({ ...values, startDate })} /><input aria-label={t("startTime")} type="time" className={`${fieldClass} min-w-0`} value={values.startTime} onChange={(event) => setValues({ ...values, startTime: event.target.value })} /></div></FormField><FormField label={t("ends")} error={fieldErrors.endsAt}><div className="grid min-w-0 grid-cols-2 gap-2"><DatePicker aria-label={t("endDate")} locale={locale} min={values.startDate} value={values.endDate} invalid={Boolean(fieldErrors.endsAt)} onValueChange={(endDate) => setValues({ ...values, endDate })} /><input aria-label={t("endTime")} type="time" className={`${fieldClass} min-w-0`} value={values.endTime} onChange={(event) => setValues({ ...values, endTime: event.target.value })} /></div></FormField></>}</div>
    <FormField label={t("invitees")} error={fieldErrors.attendeeIds}><InviteePicker people={appropriatePeople} selectedIds={values.attendeeIds} onChange={(attendeeIds) => setValues({ ...values, attendeeIds })} /><span className="text-xs font-normal text-[var(--ui-text-muted)]">{t("inviteesHelp")}</span></FormField>
