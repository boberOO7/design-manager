...
import type { CalendarEventInvitationStatus, CalendarEventType, CalendarFilters, CalendarItem, CalendarPageData, CalendarPerson, CalendarProject, CalendarView, TimeOffRequestType } from "@/types/calendar";
...
function EventForm({ isOpen, onExited, data, item, initialDate, onClose, onSaved }: { isOpen: boolean; onExited: () => void; data: CalendarPageData; item?: Extract<CalendarItem, { source: "calendar_event" }>; initialDate?: string; onClose: () => void; onSaved: (item: Extract<CalendarItem, { source: "calendar_event" }>) => void }) {
  ...
  const initial = { ...rawInitial, eventType: item?.eventType ?? allowedEventTypes[0], ...(item?.eventType === "site_visit" ? { allDay: false, endDate: rawInitial.startDate } : {}) };
  ...
  return <DetailPanel isOpen={isOpen} onExited={onExited} title={item ? t("editEventTitle") : t("addEventTitle")} eyebrow={t("eventForm")} onClose={requestClose}><form autoComplete="off" className="space-y-4" aria-busy={pending} onSubmit={(event) => event.preventDefault()}>
    ...
    <div className={isWorkMakeup || isSiteVisit || isBusinessTrip ? "grid gap-4" : "grid gap-4 sm:grid-cols-2"}><FormField label={t("type")} error={fieldErrors.eventType}><Select value={values.eventType} onValueChange={(eventType) => updateContext({ eventType: eventType as CalendarEventType })}>{[...new Set([...allowedEventTypes, item?.eventType].filter((type): type is CalendarEventType => Boolean(type)))].map((type) => <SelectItem key={type} value={type}>{type === "meeting" ? t(data.isAdmin ? "meetingPresentation" : "officeMeetingPresentation") : t(eventTypeKey[type])}</SelectItem>)}</Select></FormField>{!isWorkMakeup ? <FormField label={t("project")} optional={!isSiteVisit && !isBusinessTrip} error={fieldErrors.projectId}><Select placeholder={t("selectProject")} value={values.projectId || undefined} onValueChange={(projectId) => updateContext({ projectId, ...(isSiteVisit ? { assigneeId: data.isAdmin ? "" : data.currentUserId } : {}) })}>{(isSiteVisit ? siteVisitProjects : isBusinessTrip ? businessTripProjects : data.projects).map((project) => <SelectItem key={project.id} value={project.id} disabled={project.status === "completed"}>{project.name}{project.status === "completed" ? ` (${t("completedReopen")})` : ""}</SelectItem>)}</Select>{values.projectId && !isSiteVisit && !isBusinessTrip ? <button type="button" onClick={() => updateContext({ projectId: "" })} className="mt-2 text-left text-xs font-medium text-[var(--ui-text-secondary)] underline">{t("clearProject")}</button> : null}</FormField> : null}</div>
