  ...
  return <div className="min-w-0 space-y-5">
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      ...
      <div className="flex flex-wrap gap-2 sm:justify-end">
        <Button size="sm" className="min-h-11 sm:min-h-0" onClick={() => setDrawer({ kind: "event-form" })}><Plus className="size-4" />{t("addEvent")}</Button>
    ...
    {drawer?.kind === "day" ? <DetailPanel title={dateLabel(drawer.date, { weekday: "long", month: "long", day: "numeric", year: "numeric" }, locale)} eyebrow={t("dayDetails")} onClose={() => setDrawer(null)}>
      ...
      <div className="mt-6 flex flex-wrap gap-2"><Button size="sm" onClick={() => setDrawer({ kind: "event-form", date: drawer.date })}>{t("addEventOnDay")}</Button><Button size="sm" variant="outline" onClick={() => setDrawer({ kind: "time-off-form", date: drawer.date })}>{t("requestTimeOff")}</Button></div>
    </DetailPanel> : null}
