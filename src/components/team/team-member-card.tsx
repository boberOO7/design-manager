...
type TeamMemberCardProps = {
  ...
  vacationOpeningDays: number | null;
...
export function TeamMemberCard({ avatarUrl, birthDate, birthdayLabel, canEditProfile, city, cityGeoNamesId, countryCode, editableJobTitle, fullName, isActive, isAdmin, jobTitle, joinedAt, vacationOpeningDays, vacationOpeningDate, location, removedAt, roleLabel, systemRole, userId }: TeamMemberCardProps) {
  ...
  return <article className={`relative flex h-full w-full max-w-[18.75rem] flex-col items-center rounded-2xl border border-[var(--ui-border)] bg-[var(--ui-surface)] px-4 pb-4 pt-5 text-center ${isActive ? "" : "opacity-75"}`}>
    {isAdmin ? <StudioMemberLifecycleControls birthDate={birthDate} canEditProfile={canEditProfile} city={city} cityGeoNamesId={cityGeoNamesId} countryCode={countryCode} isFormer={!isActive} jobTitle={editableJobTitle} joinedAt={joinedAt} vacationOpeningDays={vacationOpeningDays} vacationOpeningDate={vacationOpeningDate} name={fullName} systemRole={systemRole} userId={userId} /> : null}
    ...
    <div className="flex min-w-0 w-full flex-1 flex-col pt-4">
      ...
      <div className="mt-auto flex flex-wrap items-center justify-center gap-x-3 gap-y-2 border-t border-[var(--ui-border-subtle)] pt-3 text-xs">
