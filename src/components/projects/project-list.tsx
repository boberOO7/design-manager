...
function ProjectDesktopRow({ project }: { project: ProjectItem }) {
  ...
  return <li><Link href={getProjectHref(project.id)} className="grid min-h-24 grid-cols-[minmax(14rem,1.55fr)_minmax(13rem,1fr)_minmax(10rem,0.85fr)_minmax(12rem,1fr)_auto] items-center gap-4 px-4 py-3 transition-colors hover:bg-stone-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus)]"><ProjectIdentity project={project} /><div className="flex flex-wrap gap-1.5"><Badge className={health.className} label={getProjectHealthLabel(project.health)} /><Badge className={lifecycle.className} label={lifecycle.label} /><Badge className={priority.className} label={priority.label} /></div><ProjectProgress project={project} compact /><ProjectDeadlines project={project} /><span className="ui-numeric text-sm font-medium text-[var(--ui-text-secondary)]">{project.progress.openTaskCount} open<span className="sr-only"> tasks</span></span></Link></li>;
}
...
function ProjectMobileCard({ project }: { project: ProjectItem }) {
  ...
  const lifecycle = getProjectLifecycleBadgeStyle(project.status);
  const priority = getPriorityBadgeStyle(project.priority);
  return <li><Link href={getProjectHref(project.id)} className="block min-h-44 rounded-[var(--ui-radius-panel)] border border-[var(--ui-border)] bg-[var(--ui-surface)] p-4 transition-colors hover:border-[var(--ui-border-strong)] hover:bg-stone-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]"><div className="flex items-start justify-between gap-3"><ProjectIdentity project={project} /><span className="shrink-0 text-xs font-medium text-[var(--ui-text-secondary)]">Open project</span></div><div className="mt-3 flex flex-wrap gap-1.5"><Badge className={health.className} label={getProjectHealthLabel(project.health)} /><Badge className={lifecycle.className} label={lifecycle.label} /><Badge className={priority.className} label={priority.label} /></div><ProjectProgress project={project} /><ProjectDeadlines project={project} /></Link></li>;
}
