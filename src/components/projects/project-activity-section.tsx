...
import { formatActivityValue, formatRelativeTime, getActivityChangeText, groupActivityByLocalDate, isActivityChanges } from "@/lib/project-activity";
...
async function ActivityRow({ activity, locale, projectId }: { activity: ProjectActivity; locale: string; projectId: string }) {
  ...
  const detail = change ?? (memberId ? t("member", { name: formatActivityValue(memberId) }) : null);
