...
export function ProfileAvatarEditor({ avatarUrl, city, cityGeoNamesId, countryCode: initialCountryCode, fullName, userId }: ProfileAvatarEditorProps) {
  ...
  return <>
    ...
    <Dialog closeDisabled={isLocationPending} closeLabel={t("closeProfilePhoto")} description={t("profileEditorDescription")} isOpen={isOpen} onRequestClose={closeDialog} title={t("profileEditor")}>
      <div className="space-y-5 overflow-y-auto p-4 sm:p-6">
        <section aria-labelledby="profile-photo-heading">
          ...
          <input ref={fileInputRef} accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={isLocationPending} onChange={(event) => { const file = event.target.files?.[0]; event.target.value = ""; if (file) void uploadPhoto(file); }} type="file" />
        ...
        <section aria-labelledby="profile-location-heading" className="border-t border-[var(--ui-border-subtle)] pt-5">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <label className="grid gap-2 text-sm font-medium text-[var(--ui-text)]">
              {t("country")}
