...
import { BellRing, CakeSlice, CalendarDays, LoaderCircle, LockKeyhole, MapPin, Trash2, Upload } from "lucide-react";
...
export function ProfileEditorDialog({ avatarUrl: currentAvatarUrl, birthDate, city, cityGeoNamesId, countryCode: initialCountryCodeProp, fullName, joinedAt, notificationPopupsEnabled, notificationSoundEnabled, systemRole, userId, isOpen, googleCalendarResult, onClose, onAvatarChanged, returnFocusRef }: ProfileAvatarEditorProps & { isOpen: boolean; googleCalendarResult: string | null; onClose: () => void; onAvatarChanged: (url: string | undefined) => void; returnFocusRef: RefObject<HTMLButtonElement | null> }) {
  ...
  return (
    <Dialog returnFocusRef={returnFocusRef} closeDisabled={isProfilePending || Boolean(cropFile)} closeLabel={t("closeProfilePhoto")} description={cropFile ? t("cropAvatarDescription") : t("profileEditorDescription")} isOpen={isOpen} onRequestClose={closeDialog} title={cropFile ? t("cropAvatar") : t("profileEditor")}>
