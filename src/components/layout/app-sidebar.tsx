...
export function AppSidebar({ systemRole }: { systemRole: string }) {
  ...
  return (
    <aside
      className={cn(
        "hidden shrink-0 overflow-hidden border-r border-[var(--ui-border)] bg-[var(--ui-surface-subtle)] [width:var(--sidebar-width)] transition-[width] duration-[320ms] ease-[cubic-bezier(0.65,0,0.35,1)] lg:flex lg:flex-col",
      )}
    ...
    >
      <div className="flex h-[var(--ui-shell-header-height)] shrink-0 items-center border-b border-[var(--ui-border)] px-2">
        ...
        >
          <StudioFlowMark className={cn("absolute left-[17.5px] h-7 text-[var(--ui-text)] transition-[opacity,transform] duration-[320ms] ease-[cubic-bezier(0.65,0,0.35,1)]", isExpanded ? "scale-90 opacity-0" : "scale-100 opacity-100")} />
          <SpaceLogoFull className={cn("absolute left-[17.5px] h-10 w-[91px] transition-[opacity,transform] duration-[320ms] ease-[cubic-bezier(0.65,0,0.35,1)]", isExpanded ? "translate-x-0 opacity-100" : "translate-x-1 opacity-0")} />
        </Link>
      </div>
        ...
        {items.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} title={t(item.messageKey)} className={cn("relative flex min-h-11 items-center overflow-hidden rounded-[var(--ui-radius-control)] text-sm font-medium transition-[background-color,color] duration-[200ms] ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]", active ? "bg-[var(--ui-action-primary)] text-[var(--ui-action-primary-text)]" : "text-[var(--ui-text-secondary)] hover:bg-[var(--ui-surface-strong)] hover:text-[var(--ui-text)]")}>
              ...
              <span className={cn("ml-[52px] shrink-0 whitespace-nowrap transition-[opacity,transform] duration-[320ms] ease-[cubic-bezier(0.65,0,0.35,1)]", isExpanded ? "translate-x-0 opacity-100" : "translate-x-[-0.25rem] opacity-0")}>
                {t(item.messageKey)}
              </span>
            </Link>
          );
      ...
      <div className={cn("shrink-0 overflow-hidden border-t border-[var(--ui-border)] transition-[max-height,opacity,border-color] duration-[320ms] ease-[cubic-bezier(0.65,0,0.35,1)]", isExpanded ? "max-h-12 opacity-100" : "max-h-0 border-transparent opacity-0")}>
