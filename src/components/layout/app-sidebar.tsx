...
export function AppSidebar({ systemRole }: { systemRole: string }) {
  ...
  const blurAfterPointerNavigation = (event: PointerEvent<HTMLAnchorElement>) => {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "absolute inset-y-0 left-0 z-40 flex overflow-hidden border-r border-[var(--ui-border)] bg-[var(--ui-surface-subtle)] [width:var(--sidebar-width)] transition-[width,box-shadow] duration-[320ms] ease-[cubic-bezier(0.65,0,0.35,1)] lg:flex-col",
          isExpanded && "shadow-[var(--ui-shadow-popover)]",
        )}
      ...
      >
        <div className="flex h-[var(--ui-shell-header-height)] shrink-0 items-center border-b border-[var(--ui-border)] px-2">
          <Link
            href="/dashboard"
            aria-label={t("home")}
            onPointerUp={blurAfterPointerNavigation}
