  ...
  return <>
    ...
    <Drawer isOpen={open} onClose={requestClose} returnFocusRef={triggerRef} initialFocusRef={closeRef} title="Notifications" className="w-[calc(100%-1rem)] max-w-md sm:w-full" >
      <div id="notifications-drawer" className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="flex-1 overflow-y-auto">{visible.length ? visible.map((item) => { const Icon = iconFor(item.notification_type); return <button key={item.id} type="button" disabled={pending !== null} onClick={async () => { if (await readOne(item)) { setOpen(false); router.push(item.href); } }} className={`flex w-full gap-3 border-b border-[var(--ui-border-subtle)] p-5 text-left hover:bg-[var(--ui-surface-subtle)] ${item.read_at ? "" : "bg-[var(--ui-surface-muted)]"}`}><Icon className="mt-0.5 size-4 text-[var(--ui-text-muted)]"/><span className="min-w-0 flex-1"><span className="flex justify-between gap-3"><strong className="text-sm text-[var(--ui-text)]">{item.title}</strong><time className="shrink-0 text-xs text-[var(--ui-text-muted)]">{relativeTime(item.created_at)}</time></span><span className="mt-1 block text-sm leading-5 text-[var(--ui-text-secondary)]">{item.body}</span></span>{item.read_at ? null : <span className="mt-2 size-2 rounded-full bg-[var(--ui-action-primary)]"/>}</button>; }) : <p className="p-8 text-center text-sm text-[var(--ui-text-muted)]">{filter === "unread" ? "You’re all caught up." : "No notifications yet."}</p>}</div>
      </div>
    </Drawer>
  </>;
}
