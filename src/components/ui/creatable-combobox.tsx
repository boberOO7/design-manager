...
export function CreatableCombobox({ createLabel, emptyLabel, maxLength = 160, name, options, showLabel, value, onBlur, onValueChange }: {
  ...
  const suggestions = filterCreatableSuggestions(options, query);
  const canCreate = Boolean(query && !options.some((option) => option.toLocaleLowerCase() === query.toLocaleLowerCase()));
  ...
  function handleKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    if (event.key === "ArrowDown" || event.key === "ArrowUp") {
      ...
      event.preventDefault();
      choose(activeIndex < suggestions.length ? suggestions[activeIndex] : query);
  ...
  return <PopoverPrimitive.Root modal={false} open={open} onOpenChange={setOpen}>
    <PopoverPrimitive.Anchor asChild><div className="relative">
      <input ref={setInputNode} aria-activedescendant={open && activeIndex >= 0 ? optionId(activeIndex) : undefined} aria-autocomplete="list" aria-controls={listboxId} aria-expanded={open} autoComplete="off" className={cn(inputClassName, "pr-11")} maxLength={maxLength} name={name} onBlur={(event) => onBlur?.(event.currentTarget.value)} onChange={(event) => { onValueChange(event.target.value); setOpen(true); setActiveIndex(0); }} onClick={openSuggestions} onKeyDown={handleKeyDown} role="combobox" value={value} />
