...
export function SubmissionsWorkspace({ currentUserId, isAdmin, items, members }: { currentUserId: string; isAdmin: boolean; items: SubmissionSummary[]; members: SubmissionPerson[] }) {
  ...
  // Keep received comments across drawer unmounts without reloading the inbox.
  const [receivedComments, setReceivedComments] = useState(new Map<string, SubmissionComment[]>());
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-1 overflow-x-auto rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-muted)] p-1" role="tablist" aria-label={t("filters.label")}>
...
function SubmissionRow({ currentUserId, isAdmin, item, members, onOpen }: { currentUserId: string; isAdmin: boolean; item: SubmissionSummary; members: SubmissionPerson[]; onOpen: () => void }) {
  ...
  const person = item.responsible ?? item.author;
...
function CreateSubmissionDialog({ isOpen, onClose, onCreated }: { isOpen: boolean; onClose: () => void; onCreated: (id: string | null, anonymous: boolean) => void }) {
  ...
  const [requestCategory, setRequestCategory] = useState<SubmissionRequestCategory | "">("");
  ...
  return <Dialog closeDisabled={pending} closeLabel={t("close")} description={t("form.description")} isOpen={isOpen} onRequestClose={(reason) => { if (reason !== "outside" && !pending) onClose(); }} title={t("form.title")}>
    <form action={action} className="grid min-h-0 gap-5 overflow-y-auto p-5 sm:p-6">
      ...
      <div className="flex min-h-0 items-start sm:min-h-[7.5rem]">
        {type === "request" ? <FormField className="w-full" label={t("form.category")}><Select name="requestCategory" placeholder={t("form.categoryPlaceholder")} required value={requestCategory} onValueChange={(value) => { const category = SUBMISSION_REQUEST_CATEGORIES.find((candidate) => candidate === value); if (category) setRequestCategory(category); }}>{SUBMISSION_REQUEST_CATEGORIES.map((value) => <SelectItem key={value} value={value}>{t(`categories.${value}`)}</SelectItem>)}</Select></FormField> : null}
        {type === "complaint" ? <label className="flex w-full cursor-pointer gap-3 rounded-[var(--ui-radius-control)] border border-[var(--ui-border)] bg-[var(--ui-surface-muted)] p-4"><input type="checkbox" name="anonymous" className="mt-1 size-4 accent-[var(--ui-action-primary)]" /><span><span className="block text-sm font-semibold">{t("form.anonymous")}</span><span className="mt-1 block text-xs leading-5 text-[var(--ui-text-secondary)]">{t("form.anonymousNotice")}</span></span></label> : null}
      </div>
...
function SubmissionDetailDrawer({ receivedComments, loadDetail, currentUserId, isAdmin, item, members, onComment, onClose }: { receivedComments: SubmissionComment[]; loadDetail: DetailLoader; onComment: (submissionId: string, entry: SubmissionComment) => void; currentUserId: string; isAdmin: boolean; item: SubmissionSummary | null; members: SubmissionPerson[]; onClose: () => void }) {
  ...
  const knownCommentIdsRef = useRef(new Set(comments.map((entry) => entry.id)));
  ...
  const upsertComment = useCallback((entry: SubmissionComment, keepNewestVisible: boolean) => {
    ...
    if (submissionId) onComment(submissionId, entry);
...
function RowPerson({ label, person }: { label: string; person: SubmissionPerson }) { return <span className="flex min-w-0 items-center gap-2"><UserAvatar decorative imageUrl={person.avatarUrl} name={person.fullName} size="boardCard" /><span className="min-w-0"><span title={person.fullName} className="block truncate text-sm font-semibold leading-4 text-[var(--ui-text)]">{person.fullName}</span><span className="mt-0.5 block text-[11px] font-medium leading-3 text-[var(--ui-text-muted)]">{label}</span></span></span>; }
function AnonymousPerson({ label, value }: { label: string; value: string }) { return <div className="flex min-w-0 items-center gap-2"><span className="flex size-7 shrink-0 items-center justify-center rounded-full border border-[var(--ui-border-strong)] bg-[var(--ui-surface-muted)] text-[var(--ui-text-muted)]"><LockKeyhole className="size-3.5" aria-hidden="true" /></span><span className="min-w-0"><span className="block truncate text-sm font-semibold leading-4 text-[var(--ui-text)]">{value}</span><span className="mt-0.5 block text-[11px] font-medium leading-3 text-[var(--ui-text-muted)]">{label}</span></span></div>; }
