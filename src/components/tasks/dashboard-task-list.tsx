    ...
    <div>
      ...
      {items.map((task) => {
        ...
        return <li key={task.id} className="px-3 py-2.5 sm:px-4">
          ...
          <div className="-mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs"><Link href={`/projects/${task.project_id}`} className="break-words text-[var(--ui-text-muted)] hover:text-[var(--ui-text)] hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]">{task.project.name}</Link><span aria-hidden="true" className="text-[var(--ui-text-muted)]">·</span><span className={`rounded-full px-1.5 py-0.5 font-medium ${getPriorityBadgeStyle(task.priority).className}`}>{priority(task.priority)}</span>{task.due_date ? <><span aria-hidden="true" className="text-[var(--ui-text-muted)]">·</span><span className={overdue ? "font-medium text-[var(--ui-danger-text)]" : dueToday ? "font-medium text-[var(--ui-warning-text)]" : "text-[var(--ui-text-secondary)]"}>{overdue ? t("overdue") : dueToday ? t("dueToday") : t("dueDate", { date: formatDate(task.due_date, locale) })}</span></> : null}</div>
        </li>;
      })}
      </ul> : null}
    </div>
