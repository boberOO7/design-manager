  ...
  return (
    <>
      ...
      <dialog
        ...
        className="m-auto max-h-[calc(100dvh-2rem)] w-[min(92vw,34rem)] overflow-y-auto rounded-[var(--ui-radius-drawer)] border border-[var(--ui-border)] bg-[var(--ui-surface)] p-0 text-[var(--ui-text)] shadow-2xl backdrop:bg-stone-900/45"
      >
        ...
        <form ref={formRef} action={formAction} onSubmit={(event) => { if (hasSubmittedRef.current) event.preventDefault(); else hasSubmittedRef.current = true; }} className="space-y-4 p-5">
          ...
          <div className="sticky bottom-0 -mx-5 flex justify-end gap-3 border-t border-stone-100 bg-[var(--ui-surface)] px-5 pt-4 pb-1">
