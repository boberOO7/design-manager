...
export function ProjectStageConfigurationDialog({ includeInProductivity, onClose, onSaved, projectId, showProgress, stages, stageLabels }: { includeInProductivity: boolean; showProgress: boolean; onClose: () => void; onSaved: (stages: ConfiguredProjectStage[], includeInProductivity: boolean, showProgress: boolean) => void; projectId: string; stages: ConfiguredProjectStage[]; stageLabels: (stage: TaskStage) => string }) {
  ...
  const [progressVisible, setProgressVisible] = useState(showProgress);
  ...
  async function save() {
    ...
    const response = await fetch(`/api/projects/${projectId}/stage-configuration`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stages: draft, include_in_productivity: included, show_progress: progressVisible }) });
