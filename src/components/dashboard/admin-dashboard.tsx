...
import { ArrowUpRight, Banknote, CalendarDays, Check, CircleAlert, ClipboardClock, FolderKanban, ListChecks, PhoneCall, type LucideIcon } from "lucide-react";
...
import { selectAdminUpcoming, type AdminUpcomingItem } from "@/lib/dashboard";
...
export async function AdminDashboardView({ administration, dashboard, operations }: {
  ...
  const attention: AttentionItem[] = [
    ...
    ...(operations?.finance?.overdueReceivableCount ? [{ key: "receivables", title: t("attentionReceivables", { count: operations.finance.overdueReceivableCount }), context: t("domainFinance"), href: "/finance/expected?filter=receivables", icon: Banknote }] : []),
    ...(operations?.finance?.overdueObligationCount ? [{ key: "obligations", title: t("attentionObligations", { count: operations.finance.overdueObligationCount }), context: t("domainFinance"), href: "/finance/expected?filter=obligations", icon: Banknote }] : []),
  ...
  const upcoming = selectAdminUpcoming([
    ...dashboard.deadlines.flatMap((deadline): AdminUpcomingItem[] => deadline.kind === "task" && deadline.project ? [{
      ...
      context: deadline.project.name, href: `/projects/${deadline.project.id}?task=${deadline.id}`, projectId: deadline.project.id,
    ...
    ...(operations?.finance?.upcoming ?? []).map((item): AdminUpcomingItem => ({
      key: `finance:${item.id}`, kind: "finance", date: item.date, title: item.description,
      direction: item.direction,
      context: item.obligationKind === "payroll" ? t("upcomingPayroll") : item.direction === "incoming" ? t("upcomingIncomingPayment") : t("upcomingOutgoingPayment"),
      href: `/finance/expected?item=${item.id}`,
    })),
  ...
  return <div className="dashboard-shell space-y-5 overflow-x-hidden">
    <section className="dashboard-reveal relative overflow-hidden rounded-[calc(var(--ui-radius-panel)+0.25rem)] border border-[var(--ui-border)] bg-[var(--ui-surface)] shadow-[var(--ui-shadow-panel)]">
    ...
    <div className="grid grid-flow-dense items-start gap-5 xl:grid-cols-12">
      <DashboardSection className="dashboard-reveal dashboard-reveal-delay-1 xl:col-span-7" title={t("needsAttention")}>
      ...
      <DashboardSection className="dashboard-reveal dashboard-reveal-delay-2 xl:col-span-5" title={t("upcoming")}>
        <OperationalSurface className="p-1.5">{upcoming.length ? <ul className="space-y-1">{upcoming.map((item) => <UpcomingRow key={item.key} item={item} locale={locale} />)}</ul> : <EmptyState compact className="border-0 bg-transparent" title={t("emptyUpcoming")} />}</OperationalSurface>
      </DashboardSection>
      <div id="team-workload" className="dashboard-reveal dashboard-reveal-delay-2 scroll-mt-5 xl:col-span-7"><DashboardSection title={t("teamWorkload")}><OperationalSurface className="p-1.5">{workload.length ? <TeamWorkload asOf={dashboard.asOf} members={workload} today={dashboard.today} /> : <EmptyState compact className="border-0 bg-transparent" title={t("emptyWorkload")} />}</OperationalSurface></DashboardSection></div>
      <aside className="grid min-w-0 gap-5 xl:col-span-5">
        <AttentionProjects dashboard={dashboard} />
        <MyTasks dashboard={dashboard} />
...
async function MyTasks({ dashboard }: { dashboard: AdminDashboard }) {
  const t = await getTranslations("Dashboard");
  return <DashboardSection className="dashboard-reveal dashboard-reveal-delay-3" title={t("myTasks")} description={t("myTasksDescription")}><OperationalSurface className="p-1.5 [&_li]:rounded-[calc(var(--ui-radius-panel)-0.125rem)] [&_li]:transition-colors [&_li:hover]:bg-[var(--ui-surface-subtle)]">{dashboard.myTasks.length ? <DashboardTaskList currentUserId={dashboard.profile.id} tasks={dashboard.myTasks} /> : <EmptyState compact className="border-0 bg-transparent" title={t("emptyMyTasks")} />}</OperationalSurface></DashboardSection>;
}
...
function UpcomingRow({ item, locale }: { item: AdminUpcomingItem; locale: string }) {
  const iconByKind: Record<AdminUpcomingItem["kind"], LucideIcon> = { task: ListChecks, project: FolderKanban, finance: Banknote, crm: PhoneCall };
  ...
  return <li><Link href={item.href} className="group grid min-h-[4.5rem] grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-[calc(var(--ui-radius-panel)-0.125rem)] px-3 py-2.5 transition-colors duration-200 hover:bg-[var(--ui-surface-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus-ring)]">
    ...
    <span className="min-w-0"><b className="block truncate text-sm">{item.title}</b>{item.context ? <small className="mt-0.5 block truncate text-[var(--ui-text-muted)]">{item.context}</small> : null}</span>
    <time dateTime={item.date} className="ui-numeric text-right text-xs font-medium text-[var(--ui-text-secondary)]">{dateLabel}</time>
  </Link></li>;
}
