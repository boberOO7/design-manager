...
export function AdministrationWorkspace({ initialData, requestId }: { initialData: AdministrationModel; requestId?: string }) {
  ...
  async function decide(request: AdministrationRequest, action: "approve" | "reject", reviewNote = "") {
    ...
    try {
      ...
      return true;
    } catch { setError(timeOff("requestUpdateFailed")); } finally {
  ...
  return <div className="space-y-6">
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.7fr)_minmax(20rem,1fr)] xl:items-start">
      <Panel id="requests" className="p-4 sm:p-5">
        ...
        {data.pendingRequests.length ? <ul className="divide-y divide-[var(--ui-border)]">{data.pendingRequests.map((request) => <PendingRequestRow key={request.id} request={request} isPending={pendingRequestId === request.id} onApprove={(trigger) => { void decide(request, "approve").then((success) => { if (!success) openRequest(request, trigger, true); }); }} onOpen={(trigger) => openRequest(request, trigger)} />)}</ul> : <EmptyState className="mt-4" compact title={t("noRequests")} description={t("noRequestsDescription")} />}
      </Panel>
...
function DecisionRow({ request }: { request: AdministrationRequest }) {
  const timeOff = useTranslations("TimeOff"); const administration = useTranslations("Administration"); const locale = useLocale();
  const statusStyle = getTimeOffStatusBadgeStyle(request.status);
  return <li className="py-3"><p className="break-words text-sm font-medium text-[var(--ui-text-secondary)]">{request.employeeName}</p><p className="mt-1 break-words text-sm text-[var(--ui-text-muted)]">{timeOff(typeKey(request.requestType))} · {formatAdministrationDateRange(request, locale)}</p><div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1"><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusStyle.className}`}>{statusStyle.label}</span>{request.reviewerName ? <p className="break-words text-xs text-[var(--ui-text-muted)]">{administration("reviewedBy", { name: request.reviewerName })}</p> : null}</div></li>;
}
