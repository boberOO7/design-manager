    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(20rem,1fr)] xl:items-start">
      <Panel id="requests" className="p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--ui-border)] pb-4">
          ...
          <span className="ui-numeric rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800">{data.pendingRequests.length} pending</span>
        </div>
...
function PendingRequestRow({ request, isPending, onApprove, onOpen }: { request: AdministrationRequest; isPending: boolean; onApprove: (trigger: HTMLButtonElement) => void; onOpen: (trigger: HTMLButtonElement) => void }) {
  ...
  return <li className="grid gap-3 py-4 md:grid-cols-[minmax(0,1.3fr)_minmax(10rem,0.8fr)_auto] md:items-center md:gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="break-words font-semibold text-[var(--ui-text)]">{request.employeeName}</p><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusStyle.className}`}>{statusStyle.label}</span></div><p className="mt-1 break-words text-sm text-[var(--ui-text-secondary)]">{labels[request.requestType]} · {formatAdministrationDateRange(request)}</p><p className="mt-1 text-xs text-[var(--ui-text-muted)]">{request.employeeRole ?? "No job title"}</p></div><div className="min-w-0 text-sm text-[var(--ui-text-secondary)]"><p className="text-xs font-semibold uppercase tracking-[.1em] text-[var(--ui-text-muted)]">Reason</p><p className="mt-1 whitespace-pre-wrap break-words">{request.privateNote || "No private note"}</p></div><div className="flex flex-wrap gap-2 md:justify-end"><Button size="sm" variant="outline" className="min-h-11" disabled={isPending} onClick={(event) => onOpen(event.currentTarget)}>View details</Button><Button size="sm" className="min-h-11" disabled={isPending} onClick={(event) => onApprove(event.currentTarget)}>{isPending ? "Updating…" : "Approve"}</Button><Button size="sm" variant="outline" className="min-h-11" disabled={isPending} onClick={(event) => onOpen(event.currentTarget)}>Reject</Button></div></li>;
}

function AvailabilityRow({ request }: { request: AdministrationRequest }) {
  const statusStyle = getTimeOffStatusBadgeStyle(request.status);
  return <li className="flex gap-3 py-3"><span aria-hidden="true" className={`mt-1.5 size-2 shrink-0 rounded-full ${statusStyle.className}`} /><div className="min-w-0"><p className="break-words text-sm font-medium text-[var(--ui-text)]">{request.employeeName}</p><p className="mt-1 break-words text-sm text-[var(--ui-text-secondary)]">{labels[request.requestType]} · {formatAdministrationDateRange(request)}</p></div></li>;
}

function DecisionRow({ request }: { request: AdministrationRequest }) {
  const statusStyle = getTimeOffStatusBadgeStyle(request.status);
  return <li className="py-3"><p className="break-words text-sm font-medium text-[var(--ui-text-secondary)]">{request.employeeName}</p><p className="mt-1 break-words text-sm text-[var(--ui-text-muted)]">{labels[request.requestType]} · {formatAdministrationDateRange(request)}</p><div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1"><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusStyle.className}`}>{statusStyle.label}</span>{request.reviewerName ? <p className="break-words text-xs text-[var(--ui-text-muted)]">Reviewed by <span className="font-medium text-[var(--ui-text-secondary)]">{request.reviewerName}</span></p> : null}</div></li>;
}
