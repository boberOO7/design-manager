...
export function AdministrationWorkspace({ initialData }: { initialData: AdministrationModel }) {
  const router = useRouter();
  const [data, setData] = useState(initialData);
  const [selected, setSelected] = useState<AdministrationRequest | null>(null);
  ...
  return <div className="space-y-6">
    ...
    {selected ? <RequestDrawer request={selected} onClose={() => setSelected(null)} onUpdated={update} /> : null}
  </div>;
}
...
function RequestDrawer({ request, onClose, onUpdated }: { request: AdministrationRequest; onClose: () => void; onUpdated: (request: AdministrationRequest) => void }) {
