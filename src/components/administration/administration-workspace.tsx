...
function RequestRow({ request, onOpen, actionLabel, detail }: { request: AdministrationRequest; onOpen: () => void; actionLabel: string; detail?: string }) { const statusStyle = getTimeOffStatusBadgeStyle(request.status); return <div className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-stone-900">{request.employeeName}</p><span className={`rounded-full px-2 py-0.5 text-xs font-medium ${statusStyle.className}`}>{statusStyle.label}</span></div><p className="mt-1 text-sm text-stone-600">{labels[request.requestType]} · {formatAdministrationDateRange(request)}</p><p className="mt-1 text-xs text-stone-500">{request.employeeRole ?? "No job title"}{detail ? ` · ${detail}` : ""}</p></div><Button size="sm" variant="outline" onClick={onOpen}>{actionLabel}</Button></div>; }
