...
export function ChecklistTemplateManager({ studioId, templates }: { studioId: string; templates: StudioChecklistTemplate[] }) {
  ...
  function close() { if (isDirty && !window.confirm("Discard unsaved checklist template changes?")) return; setIsOpen(false); setDraft(null); }
  async function save() {
  ...
  return <>
    <section aria-labelledby="checklist-templates-heading" className="border-t border-[var(--ui-border)] pt-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 id="checklist-templates-heading" className="text-sm font-semibold text-[var(--ui-text)]">Checklist templates</h2><p className="mt-1 text-sm text-[var(--ui-text-muted)]">Reusable weighted stages for new tasks.</p></div><Button ref={triggerRef} size="sm" variant="outline" onClick={() => open(items[0] ?? null)}>Manage templates</Button></div>
      {items.length ? <ul className="mt-2 divide-y divide-[var(--ui-border)]">{items.map((template) => <li key={template.id} className="py-2 text-sm text-[var(--ui-text-secondary)]"><span className="font-medium text-[var(--ui-text)]">{template.name}</span><span className="text-[var(--ui-text-muted)]"> · {template.stages.length} stages · weight {getChecklistTemplateWeight(template)}{template.archivedAt ? " · Archived" : ""}</span></li>)}</ul> : <p className="mt-2 text-sm text-[var(--ui-text-muted)]">No templates yet.</p>}
    </section>
    <Drawer isOpen={isOpen} onClose={close} initialFocusRef={closeRef} returnFocusRef={triggerRef} title="Checklist templates" description="Create and manage reusable task checklists" className="w-full max-w-[34rem]">
      <header className="flex items-start justify-between gap-3 border-b border-[var(--ui-border)] px-4 py-3 sm:px-5"><div><p className="text-xs font-semibold uppercase tracking-[.14em] text-[var(--ui-text-muted)]">Studio templates</p><h2 className="mt-0.5 text-lg font-semibold text-[var(--ui-text)]">Checklist templates</h2></div><Button ref={closeRef} type="button" size="sm" variant="ghost" className="size-11 shrink-0 p-0" onClick={close} aria-label="Close checklist templates"><X className="size-4" /></Button></header>
      <main className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">{error ? <p role="alert" className="rounded-[var(--ui-radius-control)] border border-red-200 bg-red-50 p-3 text-sm text-red-800">{error}</p> : null}<div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2"><label className="sr-only" htmlFor="template-selector">Select checklist template</label><select id="template-selector" value={draft?.id ?? ""} className={inputClassName} onChange={(event) => { const template = items.find((item) => item.id === event.target.value) ?? null; if (!isDirty || window.confirm("Discard unsaved checklist template changes?")) open(template); }}><option value="" disabled>Select a template</option>{items.map((template) => <option key={template.id} value={template.id}>{template.name}{template.archivedAt ? " (Archived)" : ""}</option>)}</select><Button type="button" variant="outline" className="h-11 shrink-0 px-4" onClick={() => open(null)}><Plus className="mr-1 size-4" />New</Button></div>{draft ? <TemplateEditor draft={draft} setDraft={setDraft} isSaving={isSaving} /> : null}</main>
      <footer className="flex justify-end border-t border-[var(--ui-border)] bg-[var(--ui-surface)] px-4 py-3 sm:px-5"><Button type="button" onClick={() => void save()} disabled={!draft || isSaving}>{isSaving ? "Saving…" : "Save template"}</Button></footer>
    </Drawer>
  </>;
}
...
function SortableStageRow({ index, isSaving, onAddStage, onRemove, onUpdate, stage }: { index: number; isSaving: boolean; onAddStage: () => void; onRemove: () => void; onUpdate: (index: number, update: Partial<ChecklistTemplateStage>) => void; stage: ChecklistTemplateStage }) {
  ...
  return <li ref={dropRef} className={cn("grid min-w-0 grid-cols-[2.75rem_minmax(0,1fr)_4.5rem_2.75rem] items-center gap-1 py-1.5 transition-[background-color,opacity] duration-150", isDropTarget && "rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-muted)]", isDragging && "opacity-40")}><button ref={dragRef} type="button" disabled={isSaving} aria-label={`Reorder ${stage.title || "stage"}. Use Space then arrow keys to move.`} className={cn("flex size-11 cursor-grab items-center justify-center rounded-[var(--ui-radius-control)] text-[var(--ui-text-muted)] outline-none hover:bg-[var(--ui-surface-muted)] focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)] active:cursor-grabbing", isDragging && "cursor-grabbing")}><GripVertical className="size-4" aria-hidden="true" /></button><Input value={stage.title} maxLength={200} disabled={isSaving} className="min-w-0" placeholder="Stage title" onChange={(event) => onUpdate(index, { title: event.target.value })} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); onAddStage(); } }} /><Input type="number" min="1" max="1000" step="1" inputMode="numeric" value={stage.weight} disabled={isSaving} className="w-full" aria-label={`${stage.title || "Stage"} weight`} onChange={(event) => onUpdate(index, { weight: Number(event.target.value) })} /><Button type="button" size="sm" variant="ghost" disabled={isSaving} className="size-11 p-0 text-red-700" aria-label={`Remove ${stage.title || "stage"}`} onClick={onRemove}><Trash2 className="size-4" /></Button></li>;
}
