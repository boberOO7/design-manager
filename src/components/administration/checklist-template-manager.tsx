...
function SortableStageRow({ index, isSaving, onAddStage, onRemove, onUpdate, stage }: { index: number; isSaving: boolean; onAddStage: () => void; onRemove: () => void; onUpdate: (index: number, update: Partial<ChecklistTemplateStage>) => void; stage: ChecklistTemplateStage }) {
  ...
  return <li ref={dropRef} className={cn("grid min-w-0 grid-cols-[2.75rem_minmax(0,1fr)_4.5rem_2.75rem] items-center gap-1 py-1.5 transition-[background-color,opacity] duration-150", isDropTarget && "rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-muted)]", isDragging && "opacity-40")}><button ref={dragRef} type="button" disabled={isSaving} aria-label={`Reorder ${stage.title || "stage"}. Use Space then arrow keys to move.`} className={cn("flex size-11 cursor-grab items-center justify-center rounded-[var(--ui-radius-control)] text-[var(--ui-text-muted)] outline-none hover:bg-[var(--ui-surface-muted)] focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)] active:cursor-grabbing", isDragging && "cursor-grabbing")}><GripVertical className="size-4" aria-hidden="true" /></button><Input value={stage.title} maxLength={200} disabled={isSaving} className="min-w-0" placeholder="Stage title" onChange={(event) => onUpdate(index, { title: event.target.value })} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); onAddStage(); } }} /><Input type="number" min="1" max="1000" step="1" inputMode="numeric" value={stage.weight} disabled={isSaving} className="w-full" aria-label={`${stage.title || "Stage"} weight`} onChange={(event) => onUpdate(index, { weight: Number(event.target.value) })} /><Button type="button" size="sm" variant="ghost" disabled={isSaving} className="size-11 p-0 text-[var(--ui-danger-text)]" aria-label={`Remove ${stage.title || "stage"}`} onClick={onRemove}><Trash2 className="size-4" /></Button></li>;
}
