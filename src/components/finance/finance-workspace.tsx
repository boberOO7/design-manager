...
export function FinanceWorkspace({ settings, accounts, currencies, balances, today, canReopen = false }: { settings: FinanceSettings | null; accounts: FinanceAccount[]; currencies: FinanceCurrency[]; balances: NonNullable<Awaited<ReturnType<typeof getFinanceData>>>["balances"]; today: string; canReopen?: boolean }) {
  const t = useTranslations("Finance");
  const locale = useLocale();
  const router = useRouter();
