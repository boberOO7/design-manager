...
type PaymentEditor = { intent: "schedule"; kind: "payroll" | "recurring"; schedule?: Schedule; groupId?: string } | { intent: "stop"; schedule: Schedule } | { intent: "remove"; schedule: Schedule } | { intent: "bonus"; employeeId?: string };
...
function ScheduleForm({ data, editor, onSaved, onPending }: { data: Data; editor: PaymentEditor; onSaved: () => void; onPending: (value: boolean) => void }) {
  ...
  const editable = payroll && Boolean(data.payrollEditability.find((row) => row.schedule_id === schedule?.id)?.editable);
  ...
  const [effectiveFromMonth, setEffectiveFromMonth] = useState((editable ? current?.effective_from ?? month : schedule ? nextMonth : month).slice(0, 7));
  ...
  return <FinanceActionForm action={saveFinanceSchedule} label={t(editor.intent === "stop" ? "schedules.stop" : editor.intent === "remove" ? "schedules.removePay" : "planning.save")} onCancel={onSaved} cancelLabel={t("planning.cancel")} onSaved={onSaved} onPending={onPending}>
    ...
    {editor.intent === "remove" ? <>
      ...
      {schedule && !editable ? <p className="text-sm text-[var(--ui-text-secondary)]">{t("schedules.revisionDateHelp")}</p> : null}
      {payroll ? <>
...
export function FinanceSchedulesWorkspace(data: Data) {
  ...
  function ruleRow(schedule: Schedule) {
    ...
    const editability = data.payrollEditability.find((row) => row.schedule_id === schedule.id);
  ...
  return <div className="w-full min-w-0 space-y-6">
    ...
    <section aria-labelledby="section-payroll" className={`${panel} overflow-hidden`}>
      ...
      {payroll.length ? <div className="hidden border-b border-[var(--ui-border)] px-5 py-2 text-xs font-medium text-[var(--ui-text-muted)] lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)_minmax(0,1.3fr)_20rem] lg:gap-x-4"><span>{t("schedules.employee")}</span><span>{t("schedules.agreedAmount")}</span><span>{t("schedules.payoutTiming")}</span><span className="sr-only">{t("schedules.history")}</span></div> : null}
    ...
    <Dialog className={editor?.intent === "setup" ? "h-auto max-h-[calc(100dvh-1rem)] sm:!max-w-[90rem]" : editor?.intent === "move" || editor?.intent === "group" ? "h-auto max-h-[calc(100dvh-1rem)] max-w-lg" : undefined} isOpen={editor !== null} closeDisabled={pending} onRequestClose={close} title={t(editor?.intent === "setup" ? "schedules.setupTeam" : editor?.intent === "group" ? editor.group ? "schedules.renameGroup" : "schedules.addGroup" : editor?.intent === "move" ? "schedules.moveGroup" : editor?.intent === "bonus" ? "schedules.addBonus" : editor?.intent === "stop" ? "schedules.stop" : editor?.intent === "remove" ? "schedules.removePay" : editor?.schedule ? editor.kind === "payroll" ? data.payrollEditability.find((row) => row.schedule_id === editor.schedule?.id)?.editable ? "schedules.editPay" : "schedules.reviseCompensation" : "schedules.reviseRecurring" : editor?.kind === "payroll" ? "schedules.addCompensation" : "schedules.addRecurring")} closeLabel={t("movements.close")}>
