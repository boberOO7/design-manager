    ...
    {editor.intent === "stop" ? <>
      ...
      {payroll ? <>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_8rem_minmax(15rem,1fr)] lg:items-end">
          ...
          <div className="motion-reduce:[&_*]:transition-none"><BinarySwitch emptyLabel={t("schedules.net")} label={t("schedules.basis")} value={basis} options={["net", "gross"] as const} optionLabel={(value) => t(`schedules.${value}`)} onChange={(value) => { setBasis(value); if (value === "gross") setCostsOpen(true); }} /><input type="hidden" name="basis" value={basis} /></div>
          <AnimatedDisclosure className="sm:col-span-2 lg:col-span-3" title={t("schedules.payrollCosts")} open={costsOpen} onOpenChange={setCostsOpen}><div className="mt-2 grid gap-4 sm:grid-cols-2">
...
export function FinanceSchedulesWorkspace(data: Data) {
  ...
  const termReason = (reason: string | null) => !reason || ["Compensation agreement", "Домовленість про оплату праці", "Compensation revision", "Нова редакція оплати праці"].includes(reason) ? "" : reason;
