    ...
    {editor.intent === "stop" ? <>
      ...
      {payroll ? <>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {costChoice === "has" ? <FormField label={t("schedules.certainty")}><Select aria-label={t("schedules.certainty")} value={costStatus === "estimated" ? "estimated" : "fixed"} onValueChange={setCostStatus}>{["fixed", "estimated"].map((value) => <SelectItem key={value} value={value}>{t(`schedules.${value}`)}</SelectItem>)}</Select></FormField> : null}
