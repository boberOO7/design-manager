...
export function FinanceCashChart({ data, currency, through = data.forecast.through, compact = false }: { data: FinanceOverview; currency: FinanceCurrency; through?: string; compact?: boolean }) {
  ...
  const plotBottom = compact ? 160 : 240;
  const y = (amount: string | number) => plotBottom - (Number(amount) - min) / (max - min) * (plotBottom - 32);
  ...
  return <div className="space-y-3">
    ...
    <div ref={container}>
      {chartSafe ? <svg viewBox={`0 0 ${width} ${plotBottom + 40}`} className="w-full" role="group" aria-label={t("cashChart")} aria-describedby="cash-chart-description">
        ...
        <line x1={x(report.asOf)} x2={x(report.asOf)} y1="20" y2={plotBottom} stroke="var(--ui-text-muted)" strokeDasharray="3 4" />
        ...
        {[data.actualFrom, through].map((date, i) => <text key={i} x={x(date)} y={plotBottom + 27} textAnchor={i === 0 ? "start" : "end"} fontSize="12" fill="var(--ui-text-secondary)">{date}</text>)}
        {points.map((p, i) => p.amount === null ? null : <circle key={i} cx={x(p.date)} cy={y(p.amount)} r={selected === i ? 5 : compact ? 2 : 3} fill={p.kind === "actual" ? "var(--ui-text)" : "var(--ui-success-accent)"} stroke="transparent" strokeWidth="40" tabIndex={i === (selected ?? points.findIndex(point => point.amount !== null)) ? 0 : -1} role="button" aria-label={`${t(p.kind)} · ${p.date} · ${format(p.amount)}${p.kind === "forecast" && incomplete ? ` · ${t("incomplete")}` : ""}`} onFocus={() => setSelected(i)} onMouseEnter={() => setSelected(i)} onClick={() => setSelected(i)} onKeyDown={event => {
