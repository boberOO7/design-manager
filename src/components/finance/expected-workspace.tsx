...
export function FinanceExpectedWorkspace(props:Foundation&FinancePlanningData&{ page:number;creditPage:number;filter:string;period?:FinancePlanningPeriod;today?:string;project?:ProjectContext;itemId?:string }) {
  ...
  const renderItem=(item:FinanceExpected)=>{
    ...
    return <article key={item.id} id={`expected-${item.id}`} className="space-y-2.5 p-3 sm:p-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2"><details className="group text-sm" open={props.itemId === item.id}><summary className="flex min-h-11 cursor-pointer list-none items-center text-[var(--ui-text-secondary)] underline decoration-[var(--ui-border-strong)] underline-offset-2 marker:content-none sm:min-h-8">{t("planning.details")}</summary><div className="mt-2 space-y-3 rounded-[var(--ui-radius-control)] bg-[var(--ui-surface-subtle)] p-3">
