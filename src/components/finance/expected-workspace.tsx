  ...
  const renderItem=(item:FinanceExpected)=>{
    ...
    return <article key={item.id} id={`expected-${item.id}`} data-direction={item.direction}>
      <details className="group" open={props.itemId===item.id}><summary aria-label={t("planning.detailsNamed",{name:project?`${title} · ${project.context}`:title})} className="grid min-h-16 cursor-pointer list-none grid-cols-[minmax(0,1fr)_auto_1.25rem] items-center gap-x-3 gap-y-0.5 px-3 py-2.5 transition-colors hover:bg-[var(--ui-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus)] motion-reduce:transition-none sm:min-h-12 sm:grid-cols-[8rem_minmax(0,1fr)_minmax(8rem,auto)_minmax(9rem,auto)_1.25rem] sm:px-4 sm:py-2">
        ...
        {obligation?.obligation.kind==="payroll"?<dl className="grid gap-x-5 gap-y-2 text-xs sm:grid-cols-2 lg:grid-cols-3">
        ...
        </dl>:<dl className="grid gap-x-5 gap-y-2 text-xs sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-[var(--ui-border)] pt-2">{active?<Button asChild size="sm"><Link href={`/finance/movements?expected=${item.id}`}>{t("planning.recordPayment")}</Link></Button>:null}{tripLink?<Link className="px-2 py-1.5 text-xs underline" href={`/finance/trips/${tripLink.trip_id}`}>{t("trips.title")}</Link>:link&&!props.project?<Link className="px-2 py-1.5 text-xs underline" href={`/projects/${link.project_id}?view=finance&stream=${link.stream}`}>{link.project?.name??t("project.open")}</Link>:<Button size="sm" variant="ghost" onClick={()=>setEditing(item)}>{t("edit")}</Button>}{active?<Button size="sm" variant="ghost" onClick={()=>{setPaymentId("");setMatching(item);}}>{t("planning.match")}</Button>:null}{link&&item.commitment!=="cancelled"?<Button size="sm" variant="ghost" onClick={()=>setCancelling(item)}>{t("project.cancelExpectation")}</Button>:null}</div>
        {history.length?<div className="border-t border-[var(--ui-border)] pt-2"><p className="text-xs font-medium text-[var(--ui-text-secondary)]">{t("planning.history")}</p><ul className="mt-1 space-y-1 text-xs">{history.map((entry)=>{const remaining=entry.amount+history.filter((release)=>release.released_allocation_id===entry.id).reduce((sum,release)=>sum+release.amount,0);return <li key={entry.id} className="flex flex-wrap items-center gap-x-2 break-words"><span>{date(entry.movement.financial_date)} · {money(entry.amount,item.currency)} · {entry.amount<0?t(entry.cause_movement_id?"planning.cashRelease":"planning.manualRelease"):t("planning.matched")}</span>{entry.amount>0&&remaining>0&&!tripLink?.cash?<Button size="sm" variant="ghost" onClick={()=>setReleasing(entry.id)}>{t("planning.unmatch")}</Button>:null}</li>;})}</ul></div>:null}
      </div></details>
    </article>;
  };
