...
export function FinanceExpectedWorkspace(props:Foundation&FinancePlanningData&{ page:number;creditPage:number;filter:string;period?:FinancePlanningPeriod;today?:string;project?:ProjectContext }) {
  ...
  const periodFor=(item:FinanceExpected)=>obligationFor(item)?.obligation.period_start?.slice(0,7)??linkFor(item)?.period_start?.slice(0,7)??item.expected_payment_date?.slice(0,7)??item.due_date?.slice(0,7)??"";
  ...
  const attention:FinanceExpected[]=[],current:FinanceExpected[]=[],earlier:FinanceExpected[]=[],future=new Map<string,FinanceExpected[]>();
  for(const item of props.items){
    const itemPeriod=periodFor(item);
  ...
  const renderItem=(item:FinanceExpected)=>{
    ...
    const active=item.commitment!=="cancelled"&&(item.remaining_amount??0)>0,itemPeriod=periodFor(item);
    const context=itemPeriod?(obligation?.obligation.kind==="payroll"?t("planning.forPeriod",{period:monthLabel(itemPeriod)}):monthLabel(itemPeriod)):link?.context_label??null;
