...
export function FinanceExpectedWorkspace(props:Foundation&FinancePlanningData&{ page:number;creditPage:number;filter:string;period?:FinancePlanningPeriod;today?:string;project?:ProjectContext;itemId?:string;attention?:"overdue" }) {
  ...
  const renderItem=(item:FinanceExpected)=>{
    ...
    return <article key={item.id} id={`expected-${item.id}`} data-direction={item.direction}>
      <details className="group overflow-hidden rounded-[var(--ui-radius-control)] bg-[var(--ui-surface)] open:shadow-[var(--ui-shadow-panel)]" open={props.itemId===item.id}><summary aria-label={t("planning.detailsNamed",{name:project?`${title} · ${project.context}`:title})} className="grid min-h-20 cursor-pointer list-none grid-cols-[minmax(0,1fr)_auto_1rem] items-center gap-x-3 gap-y-1 px-3 py-3 transition-colors hover:bg-[var(--ui-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus)] motion-reduce:transition-none md:min-h-16 md:grid-cols-[8.5rem_minmax(0,1fr)_minmax(7.5rem,auto)_minmax(10rem,auto)_1rem] md:gap-x-5 md:px-4">
        ...
        <strong className={`ui-numeric col-start-2 row-span-3 row-start-1 whitespace-nowrap text-right text-sm font-semibold md:col-start-4 md:row-span-1 md:text-base ${amountClass}`}>{props.project ? "" : item.direction==="incoming"?"+":"−"}{money(item.amount,item.currency)}{props.project&&item.payment_state==="partial"?<span className="mt-1 block max-w-48 whitespace-normal text-xs font-normal text-[var(--ui-text-secondary)]">{t("planning.settlementProgress",{paid:money(item.settled_amount,item.currency),remaining:money(item.remaining_amount,item.currency)})}</span>:null}</strong>
      ...
      </summary><div className="grid gap-4 bg-[var(--ui-surface-subtle)] px-3 pb-4 pt-3 text-sm md:px-4 xl:grid-cols-[minmax(0,1fr)_minmax(12rem,17rem)]">
        ...
        {obligation?.obligation.kind==="payroll"?<dl className="grid gap-x-6 gap-y-4 text-xs sm:grid-cols-2 2xl:grid-cols-4">
          ...
          <div><dt className="text-[var(--ui-text-muted)]">{t("planning.agreementState")}</dt><dd>{t(`planning.agreementOptions.${item.commitment}`)} · {t(`planning.states.${item.certainty}`)} · {state}</dd></div>
        ...
        <div className="flex flex-wrap items-center gap-1 self-start xl:flex-col xl:items-stretch">{active?<Button asChild size="sm"><Link href={`/finance/movements?expected=${item.id}`}>{t("planning.recordPayment")}</Link></Button>:null}{tripLink?<Link className="px-2 py-1.5 text-xs underline" href={`/finance/trips/${tripLink.trip_id}`}>{t("trips.title")}</Link>:link&&!props.project?<Link className="px-2 py-1.5 text-xs underline" href={`/projects/${link.project_id}?view=finance&stream=${link.stream}`}>{link.project?.name??t("project.open")}</Link>:<Button size="sm" variant="ghost" onClick={()=>setEditing(item)}>{t("edit")}</Button>}{active?<Button size="sm" variant="ghost" onClick={()=>{setPaymentId("");setMatching(item);}}>{t("planning.match")}</Button>:null}{link&&item.commitment!=="cancelled"?<Button size="sm" variant="ghost" onClick={()=>setCancelling(item)}>{t("project.cancelExpectation")}</Button>:null}</div>
