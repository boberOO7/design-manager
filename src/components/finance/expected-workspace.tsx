...
export function FinanceExpectedWorkspace(props:Foundation&FinancePlanningData&{ page:number;creditPage:number;filter:string;period?:FinancePlanningPeriod;today?:string;project?:ProjectContext;itemId?:string }) {
  ...
  const renderItem=(item:FinanceExpected)=>{
    ...
    return <article key={item.id} id={`expected-${item.id}`} data-direction={item.direction}>
      <details className="group" open={props.itemId===item.id}><summary aria-label={t("planning.detailsNamed",{name:title})} className="grid min-h-16 cursor-pointer list-none grid-cols-[minmax(0,1fr)_auto_1.25rem] items-center gap-x-3 gap-y-0.5 px-3 py-2.5 transition-colors hover:bg-[var(--ui-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus)] motion-reduce:transition-none sm:min-h-12 sm:grid-cols-[8rem_minmax(0,1fr)_minmax(8rem,auto)_minmax(9rem,auto)_1.25rem] sm:px-4 sm:py-2">
        ...
        <div className="flex flex-wrap items-center gap-2">{tripLink?<Link className="px-3 py-2 text-sm underline" href={`/finance/trips/${tripLink.trip_id}`}>{t("trips.title")}</Link>:link&&!props.project?<Link className="px-3 py-2 text-sm underline" href={`/projects/${link.project_id}?view=finance&stream=${link.stream}`}>{t("project.open")}</Link>:<Button size="sm" variant="ghost" onClick={()=>setEditing(item)}>{t("edit")}</Button>}{link&&item.commitment!=="cancelled"?<Button size="sm" variant="ghost" onClick={()=>setCancelling(item)}>{t("project.cancelExpectation")}</Button>:null}{active?<><Button size="sm" variant="ghost" onClick={()=>{setPaymentId("");setMatching(item);}}>{t("planning.match")}</Button><Button asChild size="sm"><Link href={`/finance/movements?expected=${item.id}`}>{t("planning.recordPayment")}</Link></Button></>:null}</div>
  ...
  const secondaryFilters=[...(secondaryActive&&props.filter!=="cancelled"?["all",props.filter]:secondaryActive?["all"]:[]),"cancelled"];
