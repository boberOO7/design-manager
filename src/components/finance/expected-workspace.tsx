...
function ExpectedForm({ data,item,project,onSaved,onPending }: { data:Foundation; item?:FinanceExpected; project?:ProjectContext; onSaved:()=>void; onPending:(value:boolean)=>void }) {
  ...
  return <FinanceActionForm action={saveFinancePlanning} label={t("planning.save")} onSaved={onSaved} onPending={onPending}>
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      {project?<input type="hidden" name="direction" value="incoming"/>:<FormField label={t("movements.type")}><Select name="direction" aria-label={t("movements.type")} value={direction} onValueChange={(value)=>{setDirection(value);setCategory("");}}>{["incoming","outgoing"].map((value)=><SelectItem key={value} value={value}>{t(`movements.kinds.${value}`)}</SelectItem>)}</Select></FormField>}
      <FormField label={t("movements.amount")}><Input name="amount" defaultValue={item?.amount??(project?.stream==="supervision"&&terms?.mode==="per_visit"?terms.amount??"":"")} inputMode="decimal" required/></FormField>
  ...
  return <div className="mx-auto w-full max-w-4xl space-y-6">
    ...
    <section className={`${panel} divide-y divide-[var(--ui-border)]`} aria-label={t("planning.title")}>
      {props.items.length?props.items.map((item)=>{
        ...
        const title=link?.source==="monthly"&&item.description===`Supervision · ${link.period_start?.slice(0,7)}`?`${t("project.streams.supervision")} · ${link.period_start?.slice(0,7)}`:item.description||categoryLabel(item.category_id);
    ...
    <Dialog isOpen={editing!==null} closeDisabled={pending} onRequestClose={()=>setEditing(null)} title={t(editing==="new"?"planning.create":"planning.edit")} closeLabel={t("movements.close")}>
      {editing?<div className="p-5"><ExpectedForm data={props} project={props.project} item={editing==="new"?undefined:editing} onSaved={()=>setEditing(null)} onPending={setPending}/></div>:null}
    </Dialog>
