...
import type { getFinanceData,FinancePlanningData } from "@/data/queries/finance";
...
function ExpectedForm({ data,item,onSaved,onPending }: { data:Foundation; item?:FinanceExpected; onSaved:()=>void; onPending:(value:boolean)=>void }) {
  const t=useTranslations("Finance"),locale=useLocale();
  const [direction,setDirection]=useState(item?.direction??"incoming");
  const [currency,setCurrency]=useState(item?.currency??data.settings?.base_currency??"UAH");
  const [category,setCategory]=useState(item?.category_id??"");
  const [commitment,setCommitment]=useState(item?.commitment??"agreed");
  ...
  const effectiveEstablished=canEstablish&&(establishedTouched?established:Boolean(due));
  return <FinanceActionForm action={saveFinancePlanning} label={t("planning.save")} onSaved={onSaved} onPending={onPending}>
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <FormField label={t("movements.type")}><Select name="direction" aria-label={t("movements.type")} value={direction} onValueChange={(value)=>{setDirection(value);setCategory("");}}>{["incoming","outgoing"].map((value)=><SelectItem key={value} value={value}>{t(`movements.kinds.${value}`)}</SelectItem>)}</Select></FormField>
      <FormField label={t("movements.amount")}><Input name="amount" defaultValue={item?.amount??""} inputMode="decimal" required/></FormField>
      <FormField label={t("currency")}><FinanceCurrencySelect name="currency" currencies={data.currencies} reportingCurrency={data.settings?.base_currency??""} value={currency} onValueChange={setCurrency}/></FormField>
      <FinanceCategorySelect categories={data.categories} direction={direction} value={category} onValueChange={setCategory} currentId={item?.category_id}/>
      <FormField label={t("planning.dueDate")}><DatePicker name="dueDate" aria-label={t("planning.dueDate")} value={due} onValueChange={(value)=>{setDue(value);if(!expectedTouched)setExpected(value);}} locale={locale}/></FormField>
      ...
      <label className="flex min-h-11 items-center gap-3 self-end text-sm text-[var(--ui-text-secondary)]"><input type="checkbox" checked={estimated} onChange={(event)=>setEstimated(event.target.checked)} className="size-4"/>{t("planning.estimatedAmount")}</label>
    </div>
...
export function FinanceExpectedWorkspace(props:Foundation&FinancePlanningData&{ page:number;creditPage:number;filter:string }) {
  const t=useTranslations("Finance"),locale=useLocale();
  ...
  const categoryLabel=(id:string|null)=>{const category=props.categories.find((item)=>item.id===id);return category?.default_key&&!category.custom_name?t(`planning.defaults.${category.default_key}`):category?.name??"";};
  const date=(value:string|null)=>value?formatDateOnly(value,locale):"—";
  const href=(page=props.page,credits=props.creditPage,filter=props.filter)=>`/finance/expected?page=${page}&credits=${credits}&filter=${filter}`;
  ...
  return <div className="mx-auto w-full max-w-4xl space-y-6">
    ...
    <nav aria-label={t("planning.filters")} className="flex flex-wrap gap-x-4 gap-y-2 text-sm">{["all","incoming","outgoing","receivables","obligations","cancelled"].map((filter)=><Link key={filter} href={href(1,props.creditPage,filter)} aria-current={props.filter===filter?"page":undefined} className="text-[var(--ui-text-secondary)] aria-[current=page]:font-semibold aria-[current=page]:underline">{t(`planning.filtersList.${filter}`)}</Link>)}</nav>
    <section className={`${panel} divide-y divide-[var(--ui-border)]`} aria-label={t("planning.title")}>
      {props.items.length?props.items.map((item)=>{
        ...
        return <article key={item.id} className="space-y-3 p-4 sm:p-5">
          <div className="flex flex-wrap justify-between gap-3"><div className="min-w-0"><h2 className="break-words font-medium">{item.description||categoryLabel(item.category_id)}</h2><p className="mt-1 text-xs text-[var(--ui-text-muted)]">{t(`movements.kinds.${item.direction}`)} · {categoryLabel(item.category_id)}</p></div><strong className="ui-numeric text-sm">{money(item.amount,item.currency)}</strong></div>
          ...
          <div className="flex flex-wrap items-center gap-2"><Button variant="ghost" onClick={()=>setEditing(item)}>{t("edit")}</Button>{active?<><Button variant="ghost" onClick={()=>{setPaymentId("");setMatching(item);}}>{t("planning.match")}</Button><Link className="px-3 py-2 text-sm font-medium underline" href={`/finance/movements?expected=${item.id}`}>{t("planning.recordPayment")}</Link></>:null}</div>
    ...
    <Dialog isOpen={editing!==null} closeDisabled={pending} onRequestClose={()=>setEditing(null)} title={t(editing==="new"?"planning.create":"planning.edit")} closeLabel={t("movements.close")}>
      {editing?<div className="p-5"><ExpectedForm data={props} item={editing==="new"?undefined:editing} onSaved={()=>setEditing(null)} onPending={setPending}/></div>:null}
    </Dialog>
    <Dialog isOpen={matching!==null} closeDisabled={pending} onRequestClose={()=>setMatching(null)} title={t("planning.match")} closeLabel={t("movements.close")}>
      {matching?<div className="p-5"><FinanceActionForm action={saveFinancePlanning} label={t("planning.match")} onSaved={()=>setMatching(null)} onPending={setPending}>
        ...
        <FormField label={t("planning.payment")}><Select name="movementId" aria-label={t("planning.payment")} value={paymentId} onValueChange={setPaymentId} required searchPlaceholder={t("planning.searchPayments")} searchEmptyMessage={t("planning.noPayments")}>{candidates.map((payment)=><SelectItem key={payment.id} value={payment.id??""}>{date(payment.financial_date)} · {payment.description||payment.category} · {money(payment.unapplied_amount,payment.currency)}</SelectItem>)}</Select></FormField>
