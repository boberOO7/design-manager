...
function ExpectedForm({ data,item,project,obligation,onSaved,onPending }: { data:Foundation; item?:FinanceExpected; project?:ProjectContext; obligation?:string; onSaved:()=>void; onPending:(value:boolean)=>void }) {
  ...
  return <FinanceActionForm action={saveFinancePlanning} label={t("planning.save")} onSaved={onSaved} onPending={onPending}>
    ...
    <details className="rounded-[var(--ui-radius-control)] border border-[var(--ui-border)] text-sm text-[var(--ui-text-secondary)]"><summary className="cursor-pointer px-3 py-2.5 font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]">{t("planning.moreOptions")}</summary><div className="grid gap-4 border-t border-[var(--ui-border)] p-3 sm:grid-cols-2">
      <FormField label={t("planning.agreementState")}><Select name="commitment" aria-label={t("planning.agreementState")} value={commitment} onValueChange={setCommitment}>{(project&&item?.commitment!=="cancelled"?["agreed","tentative"]:["agreed","tentative","cancelled"]).map((value)=><SelectItem key={value} value={value}>{t(`planning.agreementOptions.${value}`)}</SelectItem>)}</Select></FormField>
      <label className="flex min-h-11 items-center gap-3 self-end"><input type="checkbox" disabled={fixedPayroll} checked={estimated} onChange={(event)=>setEstimated(event.target.checked)} className="size-4"/>{t("planning.estimatedAmount")}</label>
      {canEstablish?<div className="space-y-1.5 sm:col-span-2"><label className="flex min-h-11 items-center gap-3"><input type="checkbox" checked={effectiveEstablished} onChange={(event)=>{setEstablishedTouched(true);setEstablished(event.target.checked);}} className="size-4"/>{t(direction==="incoming"?"planning.trackReceivable":"planning.trackObligation")}</label><p className="text-xs text-[var(--ui-text-muted)]">{t("planning.establishedAdvancedHelp")}</p></div>:null}
      <div className="sm:col-span-2"><FormField label={t("movements.description")}><Textarea name="description" defaultValue={item?.description??""} rows={2} maxLength={2000}/></FormField></div>
    </div></details>
  </FinanceActionForm>;
}
...
export function FinanceExpectedWorkspace(props:Foundation&FinancePlanningData&{ page:number;creditPage:number;filter:string;period?:FinancePlanningPeriod;today?:string;project?:ProjectContext;itemId?:string }) {
  ...
  const renderItem=(item:FinanceExpected)=>{
    ...
    const title=titleFor(item),rowDate=item.expected_payment_date??item.due_date;
    ...
    const amountClass=item.direction==="incoming"?"text-[var(--ui-success-text)]":"text-[var(--ui-danger-text)]";
    return <article key={item.id} id={`expected-${item.id}`} data-direction={item.direction}>
      <details className="group" open={props.itemId===item.id}><summary aria-label={t("planning.detailsNamed",{name:title})} className="grid min-h-16 cursor-pointer list-none grid-cols-[minmax(0,1fr)_auto_1.25rem] items-center gap-x-3 gap-y-0.5 px-3 py-2.5 transition-colors hover:bg-[var(--ui-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus)] motion-reduce:transition-none sm:min-h-12 sm:grid-cols-[8rem_minmax(12rem,1fr)_minmax(8rem,auto)_minmax(9rem,auto)_1.25rem] sm:px-4 sm:py-2">
        ...
        <h3 className="col-start-1 row-start-1 min-w-0 break-words text-sm font-semibold text-[var(--ui-text)] sm:col-start-2">{title}</h3>
        ...
        <strong className={`ui-numeric col-start-2 row-span-3 row-start-1 whitespace-nowrap text-right text-sm font-semibold sm:col-start-4 sm:row-span-1 ${amountClass}`}>{item.direction==="incoming"?"+":"−"}{money(item.amount,item.currency)}</strong>
        <ChevronDown className="col-start-3 row-span-3 row-start-1 size-4 text-[var(--ui-text-muted)] transition-transform duration-150 group-open:rotate-180 motion-reduce:transition-none sm:col-start-5 sm:row-span-1" aria-hidden="true"/>
        ...
        <p className="text-xs text-[var(--ui-text-muted)]">{context?`${context} · `:""}{timing}{link?.context_label?` · ${link.context_label}`:""}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">{link&&!props.project?<Link className="px-3 py-2 text-sm underline" href={`/projects/${link.project_id}?view=finance&stream=${link.stream}`}>{t("project.open")}</Link>:<Button size="sm" variant="ghost" onClick={()=>setEditing(item)}>{t("edit")}</Button>}{link&&item.commitment!=="cancelled"?<Button size="sm" variant="ghost" onClick={()=>setCancelling(item)}>{t("project.cancelExpectation")}</Button>:null}{active?<><Button size="sm" variant="ghost" onClick={()=>{setPaymentId("");setMatching(item);}}>{t("planning.match")}</Button><Button asChild size="sm"><Link href={`/finance/movements?expected=${item.id}`}>{t("planning.recordPayment")}</Link></Button></>:null}</div>
  ...
  return <div className="mx-auto w-full max-w-7xl space-y-6">
    ...
    <Dialog isOpen={editing!==null} closeDisabled={pending} onRequestClose={()=>setEditing(null)} title={t(editing==="new"?"planning.create":"planning.edit")} closeLabel={t("movements.close")}>
      {editing?<div className="p-5"><ExpectedForm obligation={editing!=="new"?props.obligations.find((entry)=>entry.expected_item_id===editing?.id)?.component:undefined} data={props} project={props.project} item={editing==="new"?undefined:editing} onSaved={()=>{const created=editing==="new";setEditing(null);if(created&&props.itemId)router.push("/finance/expected");}} onPending={setPending}/></div>:null}
    </Dialog>
