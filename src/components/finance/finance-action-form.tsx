...
export function FinanceActionForm({ children,action,onSaved,onPending,label,cancelLabel,onCancel,submitClassName,disabled=false }: {
  ...
  cancelLabel?:string; onCancel?:()=>void; submitClassName?:string; disabled?:boolean;
}) {
  ...
  return <form action={submit} onSubmit={(event)=>event.stopPropagation()} onReset={(event)=>event.preventDefault()} className="space-y-4">
    ...
    <fieldset disabled={pending} className="min-w-0 space-y-4">{children}{onCancel&&cancelLabel?<div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={onCancel}>{cancelLabel}</Button><Button type="submit" disabled={pending||disabled} className={submitClassName}>{pending?t("movements.saving"):label}</Button></div>:<Button type="submit" disabled={pending||disabled} className={submitClassName}>{pending?t("movements.saving"):label}</Button>}</fieldset>
