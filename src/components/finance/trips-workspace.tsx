...
import { Plus, ArrowLeft, MapPin, CalendarDays, ArrowUpRight } from "lucide-react";
...
import { sumTripMoney, tripStatuses } from "@/lib/finance-trips";
...
export function FinanceTripWorkspace({ data, foundation, options, today }: { data: FinanceTripData; foundation: Foundation; options: FinanceTripOptions; today: string }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex items-start justify-between gap-3"><div><h1 className="text-xl font-semibold sm:text-2xl">{data.trip.title}</h1><p className={`mt-1 ${quiet}`}>{data.trip.destination} · {formatDateOnly(data.trip.starts_on ?? "",locale)} – {formatDateOnly(data.trip.ends_on ?? "",locale)}</p></div><Button variant="outline" onClick={()=>setEditor("trip")}>{f("edit")}</Button></div>
    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[var(--ui-text-secondary)]"><span>{t(`statuses.${data.trip.status}`)}</span><span>{data.travelers.filter(v=>v.active).map(v=>v.employee_name).join(", ")}</span>{data.trip.project_id?<Link className="inline-flex min-h-9 items-center gap-1 rounded focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]" href={`/projects/${data.trip.project_id}?view=finance`}><MapPin className="size-3.5"/>{options.projects.find(v=>v.id===data.trip.project_id)?.name}</Link>:null}{data.trip.calendar_event_id?<Link className="inline-flex min-h-9 items-center gap-1.5 rounded focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]" href={`/calendar?event=${data.trip.calendar_event_id}&date=${data.trip.starts_on}`}><CalendarDays className="size-3.5"/>{t("calendarSynced")}<ArrowUpRight className="size-3"/></Link>:null}</div>
    ...
    <div className={panel}><dl className="grid grid-cols-1 divide-y divide-[var(--ui-border)] sm:grid-cols-3 sm:divide-x sm:divide-y-0">{[[t("plan"),data.trip.planned_amount],[t("actual"),data.trip.actual_amount],[t("variance"),data.trip.variance]].map(([label,value],index)=><div key={label} className="flex min-w-0 items-baseline justify-between gap-3 px-3 py-2 sm:block sm:px-5 sm:py-4"><dt className="text-xs text-[var(--ui-text-muted)]">{label}</dt><dd className="mt-1 whitespace-nowrap text-sm font-semibold tabular-nums sm:text-xl">{index===0?<button className="rounded text-left focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]" aria-label={t("openPlan")} onClick={()=>document.querySelector<HTMLButtonElement>("[data-plan-trigger]")?.focus()}>{value===null?"—":`≈ ${money(value)}`}</button>:value===null?"—":money(value ?? "0")}</dd></div>)}</dl><div className="flex flex-wrap gap-x-5 gap-y-1 border-t border-[var(--ui-border)] px-3 py-2 text-xs text-[var(--ui-text-secondary)] sm:px-5"><span>{t("studioPaid")} <strong className="ml-1 tabular-nums">{money(data.trip.studio_paid)}</strong></span><span>{t("employeePaid")} <strong className="ml-1 tabular-nums">{money(sum(data.entries.filter(e=>e.kind==="expense" && e.employee_id)))}</strong></span>{due.length?<a className="rounded font-medium focus-visible:ring-2 focus-visible:ring-[var(--ui-focus)]" href="#trip-settlements">{t("settlementsDue",{count:due.length})}</a>:null}{!data.trip.has_plan?<span>{t("noPlan")}</span>:data.trip.plan_incomplete?<span>{t("estimateUnavailable")}</span>:null}</div></div>
    ...
    <section id="trip-settlements" className={`${panel} px-4 py-2 sm:px-5`}><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-sm font-semibold">{t("balances")}</h2>{data.trip.status!=="cancelled"?<Button size="sm" variant="outline" onClick={()=>setEditor("advance")} disabled={!data.travelers.some(v=>v.active)}>{t("issueAdvance")}</Button>:null}</div>
      ...
      {unusedAdvances.map(v=><div key={`${v.employee}-${v.currency}`} className="flex flex-wrap items-center justify-between gap-2 border-t border-[var(--ui-border)] py-2 text-sm"><span>{v.employee} · {t("advanceToReconcile")} <strong>{money(v.amount,v.currency)}</strong></span><Button size="sm" variant="ghost" onClick={()=>setEditor("trip")}>{t("reviewTripStatus")}</Button></div>)}
