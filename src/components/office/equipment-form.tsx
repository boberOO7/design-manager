  ...
  return <>
    ...
    {pc ? <>
      ...
      {section("processor", t("form.cpu"), summaries.processor || legacy.cpu, <>
        ...
        {cpu ? <div className="grid gap-4 sm:grid-cols-2"><ConfigurationSelect label={t("configuration.family")} value={cpu.family} options={CPU_FAMILIES[cpu.manufacturer]} optionLabel={familyLabel} onChange={(family) => setConfig({ ...config, processor: pcConfigurationSchema.shape.processor.parse({ ...cpu, family }) })} /><FormField label={t("form.model")} optional optionalLabel={t("optional")}><EquipmentCatalogCombobox {...autocomplete} type="cpu" manufacturer={cpu.manufacturer} family={cpu.family ?? ""} value={cpu.model ?? ""} onValueChange={(model) => setConfig({ ...config, processor: { ...cpu, model: model || null } })} /></FormField></div> : null}
        {savedText("cpu")}
      </>)}
