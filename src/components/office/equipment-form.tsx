...
import { CircuitBoard, Cpu, Fingerprint, HardDrive, MemoryStick, ChevronDown, Plus, Trash2, type LucideIcon } from "lucide-react";
...
export function EquipmentFormFields({ item, initialType, showMaintenanceFields = true, workstations, onSave, onDirtyChange, pending = false }: { pending?: boolean; onSave?: (patch: EquipmentFieldUpdate) => void; onDirtyChange?: (dirty: boolean) => void; item?: EquipmentItem; initialType?: EquipmentType; showMaintenanceFields?: boolean; workstations: WorkstationItem[] }) {
  ...
  function saveText(field: "displayName" | "manufacturer" | "model" | "serialNumber" | "notes" | "cpu" | "gpu" | "ram" | "storage", value: string) {
    ...
    const normalized = field === "displayName" ? value.trim() || item.assetTag || item.displayName : value.trim();
  ...
  const sectionIcons: Record<Section, LucideIcon> = { identification: Fingerprint, processor: Cpu, graphics: CircuitBoard, memory: MemoryStick, drives: HardDrive, specifications: CircuitBoard };
  ...
  return <>
    <div className="grid shrink-0 gap-4 sm:grid-cols-2"><FormField label={t("form.type")}><Select name="equipmentType" disabled={Boolean(onSave && configDirty)} value={type} onValueChange={(value) => { if (isEquipmentType(value)) { setType(value); setOpenSection(null); if (item && onSave) onSave({ equipmentId: item.id, field: "equipmentType", value }); } }}>{EQUIPMENT_TYPES.map((value) => <SelectItem key={value} value={value} disabled={hasSavedSpecs && value !== item?.equipmentType}>{t(`types.${value}`)}</SelectItem>)}</Select></FormField><FormField label={t("form.state")}>{item?.lifecycleState === "in_service" ? <><input type="hidden" name="lifecycleState" value="in_service" /><div className="flex min-h-11 items-center rounded-[var(--ui-radius-control)] border border-[var(--ui-border)] px-3 text-sm">{t("states.in_service")}</div></> : <Select name="lifecycleState" defaultValue={item?.lifecycleState ?? "active"} onValueChange={(value) => { if (item && onSave && (value === "active" || value === "spare" || value === "retired")) void onSave({ equipmentId: item.id, field: "lifecycleState", value }); }}>{EQUIPMENT_LIFECYCLE_STATES.filter((value) => value !== "in_service").map((value) => <SelectItem key={value} value={value}>{t(`states.${value}`)}</SelectItem>)}</Select>}</FormField></div>
    ...
    {section("identification", t("form.identification"), Object.values(identity).filter(Boolean).join(" · "), <><div className="grid gap-4 sm:grid-cols-2">{(["manufacturer", "model", "serialNumber"] as const).map((key) => pc && (key === "manufacturer" || key === "model") && !item?.[key] ? <input key={key} type="hidden" name={key} value={identity[key]} /> : <FormField key={key} label={t(`form.${key}`)} optional optionalLabel={t("optional")}><Input name={key} value={identity[key]} maxLength={160} onChange={(event) => setIdentity({ ...identity, [key]: event.target.value })} onBlur={(event) => saveText(key, event.target.value)} /></FormField>)}</div>{pc && (item?.manufacturer || item?.model) ? <p className="text-xs text-[var(--ui-text-muted)]">{t("configuration.savedIdentity")}</p> : null}</>)}
