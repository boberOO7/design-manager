...
import { CircuitBoard, Cpu, Fingerprint, Gpu, HardDrive, MemoryStick, ChevronDown, Plus, Trash2, type LucideIcon } from "lucide-react";
...
import { cpuModelSuggestions, equipmentManufacturerSuggestions, equipmentModelSuggestions, gpuModelSuggestions } from "@/lib/equipment-reference-catalog";
...
export function EquipmentFormFields({ item, initialType, showMaintenanceFields = true, workstations, onSave, onDirtyChange, pending = false }: { pending?: boolean; onSave?: (patch: EquipmentFieldUpdate) => void; onDirtyChange?: (dirty: boolean) => void; item?: EquipmentItem; initialType?: EquipmentType; showMaintenanceFields?: boolean; workstations: WorkstationItem[] }) {
  const t = useTranslations("Equipment");
  const configErrorId = useId();
  ...
  const configDirty = JSON.stringify(config) !== JSON.stringify(item?.pcConfiguration ?? EMPTY_PC_CONFIGURATION);
  ...
  const sectionIcons: Record<Section, LucideIcon> = { identification: Fingerprint, processor: Cpu, graphics: Gpu, memory: MemoryStick, drives: HardDrive, specifications: CircuitBoard };
  ...
  const manufacturerSuggestions = equipmentManufacturerSuggestions(type);
  return <>
    ...
    {section("identification", t("form.identification"), Object.values(identity).filter(Boolean).join(" · "), <><div className="grid gap-4 sm:grid-cols-2">{(["manufacturer", "model", "serialNumber"] as const).map((key) => {
      if (pc && (key === "serialNumber" || !item?.[key])) return <input key={key} type="hidden" name={key} value={identity[key]} />;
      if (key === "manufacturer" && manufacturerSuggestions.length) return <FormField key={key} label={t("form.manufacturer")} optional optionalLabel={t("optional")}><CreatableCombobox {...autocomplete} name={key} options={manufacturerSuggestions} value={identity.manufacturer} onValueChange={(manufacturer) => setIdentity({ ...identity, manufacturer })} onBlur={(value) => saveText(key, value)} /></FormField>;
      if (key === "model" && manufacturerSuggestions.length) return <FormField key={key} label={t("form.model")} optional optionalLabel={t("optional")}><CreatableCombobox {...autocomplete} name={key} options={equipmentModelSuggestions(type, identity.manufacturer)} value={identity.model} onValueChange={(model) => setIdentity({ ...identity, model })} onBlur={(value) => saveText(key, value)} /></FormField>;
    ...
    })}</div>{pc && (item?.manufacturer || item?.model) ? <p className="text-xs text-[var(--ui-text-muted)]">{t("configuration.savedIdentity")}</p> : null}</>)}
    ...
    {pc ? <>
      ...
      {section("processor", t("form.cpu"), summaries.processor || legacy.cpu, <>
        ...
        {cpu ? <div className="grid gap-4 sm:grid-cols-2"><ConfigurationSelect label={t("configuration.family")} value={cpu.family} options={CPU_FAMILIES[cpu.manufacturer]} optionLabel={familyLabel} onChange={(family) => setConfig({ ...config, processor: pcConfigurationSchema.shape.processor.parse({ ...cpu, family }) })} /><FormField label={t("form.model")} optional optionalLabel={t("optional")}><CreatableCombobox {...autocomplete} options={cpuModelSuggestions(cpu.manufacturer, cpu.family)} value={cpu.model ?? ""} onValueChange={(model) => setConfig({ ...config, processor: { ...cpu, model: model || null } })} /></FormField></div> : null}
        {savedText("cpu")}
      </>)}
      {section("graphics", t("form.gpu"), summaries.graphics || legacy.gpu, <>
        ...
        {gpu ? <><Segments label={t("configuration.vendor")} value={gpu.vendor} options={["NVIDIA", "AMD", "Intel", "Other"] as const} optionLabel={familyLabel} onChange={(vendor) => setConfig({ ...config, graphics: { mode: "discrete", details: { vendor, family: null, model: null, vramGb: null } } })} /><div className="grid gap-4 sm:grid-cols-2"><ConfigurationSelect label={t("configuration.family")} value={gpu.family} options={GPU_FAMILIES[gpu.vendor]} optionLabel={familyLabel} onChange={(family) => setConfig({ ...config, graphics: pcConfigurationSchema.shape.graphics.parse({ mode: "discrete", details: { ...gpu, family } }) })} /><FormField label={t("form.model")} optional optionalLabel={t("optional")}><CreatableCombobox {...autocomplete} options={gpuModelSuggestions(gpu.vendor, gpu.family)} value={gpu.model ?? ""} onValueChange={(model) => setConfig({ ...config, graphics: { mode: "discrete", details: { ...gpu, model: model || null } } })} /></FormField><CapacityInput label={t("configuration.vram")} value={gpu.vramGb} onChange={(vramGb) => setConfig({ ...config, graphics: { mode: "discrete", details: { ...gpu, vramGb } } })} /></div></> : null}
        {savedText("gpu")}
      </>)}
    ...
    </> : type === "laptop" ? section("specifications", t("form.specifications"), Object.values(legacy).filter(Boolean).join(" · "), <div className="grid gap-4 sm:grid-cols-2">{(["cpu", "gpu", "ram", "storage"] as const).map((key) => <FormField key={key} label={t(`form.${key}`)} optional optionalLabel={t("optional")}><Input name={key} value={legacy[key]} maxLength={500} onChange={(event) => setLegacy({ ...legacy, [key]: event.target.value })} onBlur={(event) => saveText(key, event.target.value)} /></FormField>)}</div>) : null}
    {item && onSave && pc && configDirty ? <div className="flex justify-end gap-2"><Button type="button" disabled={pending} variant="ghost" onClick={() => setConfig(item.pcConfiguration ?? EMPTY_PC_CONFIGURATION)}>{t("cancel")}</Button><Button type="button" disabled={pending} onClick={() => {
      const parsed = pcConfigurationSchema.safeParse(config);
      if (!parsed.success) { document.getElementById(configErrorId)?.focus(); return; }
      void onSave({ equipmentId: item.id, field: "pcConfiguration", value: hasPcConfiguration(parsed.data) ? parsed.data : null });
    }}>{t("configuration.save")}</Button></div> : null}
    {item && pc && configDirty && !pcConfigurationSchema.safeParse(config).success ? <p id={configErrorId} tabIndex={-1} role="alert" className="text-sm text-[var(--ui-danger-text)]">{t("configuration.invalid")}</p> : null}
