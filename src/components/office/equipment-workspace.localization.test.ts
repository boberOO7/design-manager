...
describe("Equipment localization", () => {
  it("keeps English and Ukrainian Equipment keys in parity", () => {
    ...
    for (const key of ["title", "description", "views.workstations", "views.other", "views.maintenance", "actions.addWorkstation", "actions.createWorkstations", "actions.addEquipment", "workstation.numberLabel", "workstation.unassigned", "workstation.form.quantity", "workstation.form.decreaseQuantity", "workstation.form.increaseQuantity", "workstation.form.assignEmployees", "form.displayName", "form.workstation", "form.identificationSummary", "form.assetTagDescription", "assignment.attach", "maintenance.queueTitle", "maintenance.upcoming", "maintenance.overdue", "service.send", "service.complete", "history.title", "history.types.regular_maintenance", "history.types.repair", "history.types.upgrade", "errors.numberConflict", "errors.employeeAssigned", "errors.permission"] as const) {
