...
export async function getNotificationData(): Promise<NotificationData> {
  ...
  const [itemsResult, unreadResult] = await Promise.all([
    supabase.from("notifications").select("id, notification_type, title, body, href, metadata, read_at, created_at, actor:profiles!notifications_actor_id_fkey(full_name)").eq("recipient_id", membership.authenticatedUserId).lte("created_at", now).order("created_at", { ascending: false }).limit(30),
    supabase.from("notifications").select("id", { count: "exact", head: true }).eq("recipient_id", membership.authenticatedUserId).lte("created_at", now).is("read_at", null),
  ]);
