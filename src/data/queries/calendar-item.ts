...
export async function getNormalizedCalendarEvent(eventId: string, currentUserId: string): Promise<Extract<CalendarItem, { source: "calendar_event" }> | null> {
  ...
  return {
    ...
    projectId: data.project_id, personIds: [...new Set([...invitees.map((person) => person.id), data.organizer_id, ...(data.assignee_id ? [data.assignee_id] : []), ...(data.project_id === null ? [currentUserId] : [])])],
