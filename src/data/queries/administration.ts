...
import { getUpcomingEndDate, isUpcomingAbsence, sortPendingRequests, sortRecentDecisions, sortUpcomingAbsences, type AdministrationModel, type AdministrationRequest } from "@/lib/administration";
...
export async function getAdministrationData(): Promise<AdministrationModel | null> {
  ...
  const select = "id, request_type, start_date, end_date, start_time, end_time, all_day, private_note, review_note, status, created_at, reviewed_at, cancelled_at, employee:profiles!time_off_requests_user_id_fkey!inner(full_name, job_title), reviewer:profiles!time_off_requests_reviewed_by_fkey(full_name)";
  ...
  const [pendingResult, upcomingResult, recentResult, membersResult] = await Promise.all([pendingPromise, upcomingPromise, recentPromise, membersPromise]);
  ...
  return { today, upcomingEnd, pendingRequests, upcomingAbsences, recentDecisions, team: { activeMembers: members.filter((member) => member.is_active).length, administrators: members.filter((member) => member.is_active && member.system_role === "admin").length, inactiveMembers: members.filter((member) => !member.is_active).length } };
}
