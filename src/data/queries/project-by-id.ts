...
export async function getProjectById(projectId: string): Promise<ProjectDetails | null> {
  ...
  const { data, error } = await supabase
    .from("projects")
    .select(
      "id, studio_id, name, project_code, client_name, description, total_area_m2, progress_method, status, priority, start_date, due_date, completed_at, archived_at, created_by, created_at, updated_at",
    )
    .eq("id", projectId)
    .maybeSingle();
