...
export async function getDashboardAdministration(): Promise<{ pendingCount: number; upcomingAbsences: DashboardAbsence[] } | null> {
  ...
  const [pending, upcoming] = await Promise.all([
    // Keep the Administration queue's 50-row cap and employee visibility join.
    supabase.from("time_off_requests").select("id, employee:profiles!time_off_requests_user_id_fkey!inner()")
      .eq("studio_id", membership.studio_id).eq("status", "pending").order("created_at").limit(50),
  ...
  return { pendingCount: pending.data.length, upcomingAbsences: sortUpcomingAbsences(upcoming.data.map((row) => ({
