...
export async function getDashboard(): Promise<DashboardData | null> {
  ...
  const taskRows = tasksResult.data as Array<DashboardTask & { collaborators: Array<{ profile: DashboardTask["assignee"] }> }>;
  const tasks = taskRows.map(({ collaborators, ...task }) => ({ ...task, collaborators: (collaborators as unknown as Array<{ profile: DashboardTask["assignee"] }>).flatMap(({ profile }) => profile ? [profile] : []) }));
