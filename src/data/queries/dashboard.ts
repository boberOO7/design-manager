...
export type EmployeeDashboard = { kind: "employee"; profile: { id: string; full_name: string }; metrics: { overdue: number; dueToday: number; inProgress: number; upcoming: number }; needsAttention: DashboardTaskForDrawer[]; projects: Array<DashboardProject & { openTaskCount: number; inProgressCount: number; nearestDueDate: string | null }>; hasMoreProjects: boolean; deadlines: DashboardDeadline[] };
...
export async function getDashboard(): Promise<DashboardData | null> {
  ...
  const taskQuery = supabase.from("tasks").select("id, project_id, title, description, status, priority, assignee_id, due_date, completed_at, completed_area_m2, created_at, created_by, assignee:profiles!tasks_assignee_id_fkey(id, full_name, job_title), creator:profiles!tasks_created_by_fkey(id, full_name, job_title), project:projects!tasks_project_id_fkey!inner(id, name, studio_id)").eq("project.studio_id", membership.studio_id).is("project.archived_at", null).in("project.status", ["planned", "active", "paused"]);
  ...
  const [projectsResult, tasksResult, membersResult] = await Promise.all([
    ...
    membership.system_role === "admin" ? supabase.from("studio_members").select("profile:profiles!studio_members_user_id_fkey!inner(id, full_name, job_title)").eq("studio_id", membership.studio_id).eq("is_active", true).overrideTypes<Array<{ profile: DashboardMember }>, { merge: false }>() : Promise.resolve({ data: [], error: null }),
  ]);
