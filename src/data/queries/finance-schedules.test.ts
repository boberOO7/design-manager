...
it("uses maintained open payment dates, excluding cancelled, paid and past items", async () => {
  ...
  mocks.client.mockResolvedValue(createClient<Database>("http://127.0.0.1:54321", "test-key", { auth: { persistSession: false }, global: { fetch: async (input) => {
    ...
    return new Response(JSON.stringify(url.pathname.endsWith("/finance_schedules") ? [{ id: "rule" }, { id: "empty" }] : url.pathname.endsWith("/finance_schedule_history") ? [{ schedule_id: "rule", obligations: [{ items: [
  ...
  for (const url of queries.slice(1)) expect(url.searchParams.get("studio_id")).toBe("eq.verified");
});
