...
export async function POST(request: Request) {
  ...
  const userId = membership.system_role === "admin" && parsed.data.userId ? parsed.data.userId : membership.authenticatedUserId;
  const supabase = await createClient();
  const { data, error } = await supabase.from("time_off_requests").insert({
    studio_id: membership.studio_id, user_id: userId, request_type: parsed.data.requestType,
  ...
  if (error || !data) return NextResponse.json({ success: false, formError: "The time-off request could not be created. Check the employee and date range." }, { status: 400 });
