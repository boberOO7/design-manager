...
import { AlertTriangle, ArrowRight, ClipboardCheck, Clock3, Inbox } from "lucide-react";
...
export default async function OfficeOverviewPage() {
  const [t, submissions, assignments] = await Promise.all([getTranslations("Office"), getSubmissionsData(), getOfficeAssignmentsData()]);
  ...
  const recentSubmissions = (isAdmin ? submissions.items : ownSubmissions).map((item) => ({ id: item.id, title: item.title, updatedAt: item.updatedAt, href: `/office/submissions?item=${item.id}`, kind: t("activity.submission") }));
  const recentAssignments = assignments.items.map((item) => ({ id: item.id, title: item.title, updatedAt: item.updatedAt, href: `/office/assignments?item=${item.id}`, kind: t("activity.assignment") }));
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-[var(--ui-radius-panel)] border border-[var(--ui-border)] bg-[var(--ui-surface)] shadow-[var(--ui-shadow-panel)]"><div className="border-b border-[var(--ui-border)] px-4 py-3 sm:px-5"><h3 className="font-semibold">{t("activity.title")}</h3><p className="mt-0.5 text-xs text-[var(--ui-text-muted)]">{t("activity.description")}</p></div>{recent.length ? <div className="divide-y divide-[var(--ui-border-subtle)]">{recent.map((item) => <Link key={`${item.kind}-${item.id}`} href={item.href} className="flex min-h-14 items-center justify-between gap-4 px-4 py-3 transition-colors hover:bg-[var(--ui-surface-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus)] sm:px-5"><span className="min-w-0"><span className="block truncate text-sm font-semibold">{item.title}</span><span className="block text-xs text-[var(--ui-text-muted)]">{item.kind}</span></span><ArrowRight className="size-4 shrink-0 text-[var(--ui-text-subtle)]" aria-hidden="true" /></Link>)}</div> : <p className="px-5 py-10 text-center text-sm text-[var(--ui-text-muted)]">{t("activity.empty")}</p>}</section>
  </div>;
}
