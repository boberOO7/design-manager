  ...
  return <div className="space-y-8">
    ...
    <div className="grid items-start gap-8 xl:grid-cols-[minmax(0,1.55fr)_minmax(19rem,0.85fr)]">
      <DashboardSection title="Projects requiring attention" description="Risk-ranked active projects.">
        <OperationalSurface>{dashboard.attentionProjects.length ? <ul className="divide-y divide-[var(--ui-border)]">{dashboard.attentionProjects.map((project) => <li key={project.id}><Link href={`/projects/${project.id}`} className="grid min-h-18 gap-2 px-3 py-3 transition-colors hover:bg-[var(--ui-surface-subtle)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--ui-focus)] sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-4"><div className="min-w-0"><p className="font-medium text-[var(--ui-text)]">{project.name}</p><p className="mt-1 text-sm text-[var(--ui-text-secondary)]">{getAttentionProjectReason(project)}</p></div><div className="flex flex-wrap items-center gap-2 text-xs sm:justify-end">{project.progressPercent !== null ? <span className="ui-numeric rounded-full border border-[var(--ui-border)] bg-[var(--ui-surface)] px-2 py-1 font-medium text-[var(--ui-text-secondary)]">{project.progressPercent}% progress</span> : null}<span className="ui-numeric rounded-full border border-[var(--ui-border)] bg-[var(--ui-surface-muted)] px-2 py-1 text-[var(--ui-text-secondary)]">{project.openTaskCount} open</span>{project.overdueCount ? <span className="ui-numeric rounded-full border border-[var(--ui-danger-border)] bg-[var(--ui-danger-surface)] px-2 py-1 font-medium text-[var(--ui-danger-text)]">{project.overdueCount} overdue</span> : null}{project.deadlineDaysAway !== null ? <span className="rounded-full border border-[var(--ui-warning-border)] bg-[var(--ui-warning-surface)] px-2 py-1 font-medium text-[var(--ui-warning-text)]">{project.deadlineDaysAway === 0 ? "Due today" : `Due in ${project.deadlineDaysAway}d`}</span> : null}<span className="font-medium text-[var(--ui-text-secondary)]">Open project</span></div></Link></li>)}</ul> : <EmptyState compact {...DASHBOARD_EMPTY_STATES.adminAttention} />}</OperationalSurface>
      </DashboardSection>
      <Deadlines deadlines={dashboard.deadlines} />
    </div>
