...
import { getKyivPeriodLabel, getKyivPeriodRangeLabel, getLeaderboardTotals, isLeaderboardPeriod, type LeaderboardPeriod } from "@/lib/productivity";
import { getLeaderboardBonusPercent, hasLeaderboardBonuses, type LeaderboardBonusConfig } from "@/lib/leaderboard-bonus-rules";
...
export default async function LeaderboardPage({ searchParams }: { searchParams: Promise<{ period?: string }> }) {
  ...
  const monthlyBonusesApply = period === "month" && hasLeaderboardBonuses(overview.bonusConfig);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5">
      ...
      <section aria-labelledby="period-ranking-heading" className="overflow-hidden rounded-[var(--ui-radius-panel)] border border-[var(--ui-border)] bg-[var(--ui-surface)] shadow-[var(--ui-shadow-panel)]">
        ...
        {overview.current.length === 0 ? <div className="px-5 py-10 text-center"><p className="font-medium text-[var(--ui-text)]">{t("noCompletedPeriod", { period: t(period) })}</p><p className="mt-1 text-sm text-[var(--ui-text-secondary)]">{t("noCompletedPeriodDescription")}</p></div> : <ol className="divide-y divide-[var(--ui-border)]">{overview.current.map((entry) => { const bonus = getLeaderboardBonusPercent(entry.rank, overview.bonusConfig); return <li key={entry.user_id} className={`grid grid-cols-[2.25rem_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3.5 sm:px-5 ${monthlyBonusesApply ? "sm:grid-cols-[3rem_minmax(0,1fr)_8rem_5.5rem_5.5rem]" : "sm:grid-cols-[3rem_minmax(0,1fr)_8rem_5.5rem]"}`}><p className={`ui-numeric text-sm font-semibold ${bonus > 0 ? "text-[var(--ui-text)]" : "text-[var(--ui-text-muted)]"}`}>#{entry.rank}</p><div className="flex min-w-0 items-center gap-2"><UserAvatar imageUrl={entry.avatar_url} name={entry.full_name} decorative /><div className="min-w-0"><p className="truncate font-medium text-[var(--ui-text)]">{entry.full_name}</p><p className="truncate text-sm text-[var(--ui-text-secondary)]">{entry.job_title}</p></div></div><p className="hidden text-right text-sm ui-numeric font-medium text-[var(--ui-text)] sm:block">{formatArea(entry.completed_area_m2, locale)}</p><p className="hidden text-right text-sm ui-numeric text-[var(--ui-text-secondary)] sm:block">{t("tasks", { count: entry.completed_tasks })}</p>{monthlyBonusesApply ? <div className="justify-self-end"><BonusBadge config={overview.bonusConfig} rank={entry.rank} compact t={t} /></div> : null}<div className="col-span-2 flex gap-3 text-xs text-[var(--ui-text-secondary)] sm:hidden"><span className="ui-numeric">{formatArea(entry.completed_area_m2, locale)}</span><span className="ui-numeric">{t("tasks", { count: entry.completed_tasks })}</span></div></li>; })}</ol>}
      </section>
    </div>
  );
}

function BonusBadge({ compact = false, config, rank, t }: { compact?: boolean; config: LeaderboardBonusConfig; rank: number; t: Awaited<ReturnType<typeof getTranslations>> }) {
  const bonus = getLeaderboardBonusPercent(rank, config);
