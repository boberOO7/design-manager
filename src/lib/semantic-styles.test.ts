...
describe("semantic badge styles", () => {
  it("maps every priority to its semantic variant", () => {
    expect(["low", "normal", "high", "urgent"].map((value) => getPriorityBadgeStyle(value).variant)).toEqual(["info", "neutral", "warning", "danger"]);
  });
