...
insert into public.equipment(id, studio_id, equipment_type, display_name, manufacturer, model, serial_number, cpu, storage)
values ('47000000-0000-0000-0000-000000000201', '47000000-0000-0000-0000-000000000001', 'laptop', 'Legacy Laptop', 'Lenovo', 'ThinkPad P1', 'LAPTOP-SERIAL', 'legacy laptop cpu', 'legacy laptop disk');
...
select lives_ok($$update public.equipment set pc_configuration = '{"processor":{"manufacturer":"Intel","family":"Core Ultra 7","model":"165H"},"graphics":{"mode":"integrated"},"memory":{"capacityGb":32,"generation":"DDR5","moduleCount":null},"drives":[{"type":"nvme_ssd","capacity":1,"unit":"TB"}]}' where id = '47000000-0000-0000-0000-000000000201'$$, 'laptop can use the shared structured computer configuration');
select is((select concat_ws('|', manufacturer, model, serial_number, cpu, storage) from public.equipment where id = '47000000-0000-0000-0000-000000000201'), 'Lenovo|ThinkPad P1|LAPTOP-SERIAL|legacy laptop cpu|legacy laptop disk', 'laptop identity and legacy specifications remain verbatim');
select throws_like($$update public.equipment set pc_configuration = jsonb_set(pc_configuration, '{motherboard}', '{"manufacturer":"ASUS","model":"Laptop board","chipset":null}') where id = '47000000-0000-0000-0000-000000000201'$$, '%equipment_pc_configuration_pc_components%', 'laptops cannot acquire PC-only motherboard details');
select lives_ok($$update public.equipment set pc_configuration = pc_configuration || '{"motherboard":{"manufacturer":"ASUS","model":"ProArt X670E","chipset":"X670E"},"powerSupply":{"manufacturer":"Seasonic","model":"Focus GX","wattage":850,"efficiency":"80 PLUS Gold"}}' where id = '47000000-0000-0000-0000-000000000200'$$, 'PC can store practical motherboard and power supply details');
select is((select pc_configuration#>>'{powerSupply,wattage}' from public.equipment where id = '47000000-0000-0000-0000-000000000200'), '850', 'PC power supply wattage remains structured');
