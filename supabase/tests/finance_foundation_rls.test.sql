...
select lives_ok($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Bank','UAH',1200.25)$$,'admin records precise opening balance');
select lives_ok($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Overdraft','USD',-55.50)$$,'negative opening balance allowed');
select lives_ok($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Yen','JPY',0)$$,'zero opening balance allowed');
select lives_ok($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Dinar','KWD',1.234)$$,'three-decimal currency preserved');
select lives_ok($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Unit','CLF',1.2345)$$,'four-decimal currency preserved');
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Fractional yen','JPY',1.1)$$,'%finance_balance_precision%','fractional yen rejected without rounding');
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Rounding','UAH',1.001)$$,'%finance_balance_precision%','excess cents rejected without rounding');
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Invalid','ABC',0)$$,'%finance_currency_invalid%','unknown account currency rejected');
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Invalid','USD','NaN')$$,'%check constraint%','NaN rejected');
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Invalid','USD','Infinity')$$,'%check constraint%','Infinity rejected');
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Invalid','USD',10000000000)$$,'%check constraint%','unsafe amount range rejected');
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001',' ','USD',0)$$,'%check constraint%','blank name rejected');
...
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','New cash','USD',5)$$,'%finance_new_account_zero_opening%','new account cannot rewrite finalized openings');
select lives_ok($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','New cash','USD',0)$$,'new account after cutover starts at zero');
...
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Unauthorized','USD',0)$$,'%finance_admin_required%','employee cannot create account');
...
select throws_like($$select public.save_finance_account('62000000-0000-0000-0000-000000000001','Unauthorized','USD',0)$$,'%finance_admin_required%','inactive membership cannot mutate');
set local role postgres;
