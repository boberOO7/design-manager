-- Assign a stage's eligible tasks in one transaction. Existing task triggers
-- continue to record activity and notify newly assigned people per task.
...
begin
  ...
  -- Lock the exact mutation set so a concurrent assignment or cancellation
  -- cannot reduce this bulk operation to a partial stage update.
  select coalesce(array_agg(task.id), '{}'::uuid[]) into selected_task_ids
  from (
    select id
