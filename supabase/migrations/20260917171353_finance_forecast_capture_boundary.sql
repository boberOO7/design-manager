-- Fix the balance view identifier and compare same-day cash recorded after capture.
...
select e.studio_id,e.financial_date,e.nature,
  ...
  e.reporting_amount * case when e.entry_role='fee' then -1 when coalesce(c.direction,
    case when coalesce(original.kind,m.kind)='incoming' then 'incoming' else 'outgoing' end)='incoming' then 1 else -1 end as amount,m.created_at as recorded_at
...
create or replace function public.calculate_finance_forecast(p_studio_id uuid,p_horizon text default '6',p_scenario text default 'confirmed',p_fx jsonb default '[]') returns jsonb
...
begin
  ...
  left join lateral (select sum(case when direction='incoming' then amount else -amount end) as amount from public.finance_planning_actuals
    where studio_id=p_studio_id and financial_date>=(saved.forecast->>'asOf')::date and recorded_at>saved.created_at and financial_date<=(saved.forecast->>'through')::date
