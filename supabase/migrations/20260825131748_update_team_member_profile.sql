...
begin
  ...
  set full_name = btrim(p_full_name), job_title = p_job_title, system_role = p_system_role
...
revoke all on function public.update_studio_member_profile(uuid, text, text, text) from public, anon;
grant execute on function public.update_studio_member_profile(uuid, text, text, text) to authenticated;
