-- Generic events treat the organizer like any other eligible participant. Keep
-- organizer exclusion for event types whose organizer is already a semantic role.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    if (new.user_id = event_row.organizer_id and event_row.event_type <> 'general') or new.invited_by is distinct from actor or new.status <> 'pending' or new.responded_at is not null then raise exception 'Invalid calendar invitation'; end if;
