...
begin
  ...
  insert into public.tasks(project_id,title,priority,assignee_id,created_by,stage,status) select new_project_id,t.title,'normal',a.assignee_id,auth.uid(),t.stage,'todo' from public.project_template_tasks t left join jsonb_to_recordset(p_stage_assignees) as a(stage text,assignee_id uuid) on a.stage=t.stage where t.template_id=selected_template_id order by t.stage,t.position,t.id;
