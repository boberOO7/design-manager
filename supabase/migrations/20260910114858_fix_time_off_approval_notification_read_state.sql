-- A notification's read state is one-way. Do not issue an UPDATE for a
-- submission notification that was already read: the notification trigger
-- correctly rejects every update whose old read_at is non-null.
