...
| Finance | `/finance`, `/finance/movements`, `/finance/expected`, `/finance/categories`, project `view=finance` | `src/data/queries/finance.ts`, Finance Server Actions, `src/lib/finance-fx.ts`, `src/lib/finance-planning.ts`, `src/lib/finance-projects.ts` | [finance](finance.md) |
